function SABRE2SegmModelP(JNodevalue,Massemble,JNodevalue_i,JNodevalue_j,...
   Rval,BNodevalue,LabType,axesm,vstm)
% Developed by Woo Yong Jeong.
% Date : 12/01/2012.
% ------------------------------------------------------------------------
% -------------      3D Model generator for Segment       ----------------
% ------------------------------------------------------------------------
         ...
         for i = 1:max(BNodevalue(j,:,2))
            if isequal(BNodevalue(j,i,15),1)
               plot3(axesm,BNodevalue(j,i,3),-BNodevalue(j,i,5),...
                  ...
                  'MarkerSize',8,'Tag',['M',num2str(j),'b',num2str(i)],'HitTest','on');
               hold on;
            else
...
if isequal(azf(1,1),0) && isequal(azf(1,2),0)
   view(azf(1,1), azf(1,2));
elseif isequal(azf(1,1),90) && isequal(azf(1,2),0)
   view(azf(1,1), azf(1,2));
elseif isequal(azf(1,1),0) && isequal(azf(1,2),90)
   view(azf(1,1), azf(1,2));
end
