...
nDOF    =nodedof*nelem;                                                     % Total number of dofs
...
[Kt]=StiffnessELEMENT3DESL_slender(Ae,yuc,Iux,Iy,Ip,Aft,Dt,...              % Does not currently handle releases, need to finish warping free??
    tfte,huft,Afb,tfbe,Ift,Ifb,Iwy,hufb,Ipp,Cwa,Db,L(j,1),E(i,1),wts,Rt,Rb,...
    theta1x,theta1z,theta1y,theta2x,theta2z,theta2y,theta1xp,theta2xp,...
...
[KtBB,KggBB] = assemble1(KtBB,KggBB,Kts,Kggs,Member_data,nmembers);         % Elastic stiffness & Geometric stiffness
end
end  
