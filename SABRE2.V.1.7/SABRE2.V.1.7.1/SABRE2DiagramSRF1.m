function SABRE2DiagramSRF1(JNodevalue,Massemble,JNodevalue_i,JNodevalue_j,SNodevalue,RNCc,NCc,...
   Nshe1,Nshe2,DUP1,DUP2,tau,Rval,pt_title_name,LabType,axesm,vstm)
...
[DX]=InitialEleLength(xn,xgd1,ygd1,zg1,xgd2,ygd2,zg2);
...
for i = 1:xn 
   ...
   case 1
      if isequal(Ta,0)
         % *************************** Rotation
         % bottom flage start node
         SN2=[MemLength1(i,1)+DXe(p,1) (0) (zg1(i,1)+0)];
         % top flage start node
         SN6=[MemLength1(i,1)+DXe(p,1) (0) (zg1(i,1)+0)];
         % bottom flage end node
         SN9=[MemLength1(i,1)+DXe(p+1,1) (0) (zg2(i,1)+0)];
         % top flage end node
         SN13=[MemLength1(i,1)+DXe(p+1,1) (0) (zg2(i,1)+0)];
      else  
         % *************************** Rotation
         % bottom flage start node
         SN2=[MemLength1(i,1)+DXe(p,1) (0) (zg1(i,1)+0)];
         % top flage start node
         SN6=[MemLength1(i,1)+DXe(p,1) (tau1(p,i)*xa*0.8/Ta) (zg1(i,1)+0)];
         % bottom flage end node
         SN9=[MemLength1(i,1)+DXe(p+1,1) (0) (zg2(i,1)+0)];
         % top flage end node
         SN13=[MemLength1(i,1)+DXe(p+1,1) (tau2(p,i)*xa*0.8/Ta) (zg2(i,1)+0)];
      end
      % *************************** Global Rotation
      SN2=Rz*SN2';SN2=SN2';
      SN6=Rz*SN6';SN6=SN6'; 
   ...
   if isequal(strcmp(get(vstm,'Checked'),'on'),1) % white background
      text(SN6(1,1),SN6(1,3),SN6(1,2),num2str(tau1(p,i)),'FontSize',9,...
         'Fontweight','bold','Tag','SRFT','VerticalAlignment','top','Color','k','PickableParts','none');
      text(SN13(1,1),SN13(1,3),SN13(1,2),num2str(tau2(p,i)),'FontSize',9,...
         'Fontweight','bold','Tag','SRFT','VerticalAlignment','top','Color','k','PickableParts','none'); 
   elseif isequal(strcmp(get(vstm,'Checked'),'on'),0) % black background
      text(SN6(1,1),SN6(1,3),SN6(1,2),num2str(tau1(p,i)),'FontSize',9,...
         'Fontweight','bold','Tag','SRFT','VerticalAlignment','top','Color','w','PickableParts','none');
      text(SN13(1,1),SN13(1,3),SN13(1,2),num2str(tau2(p,i)),'FontSize',9,...
         'Fontweight','bold','Tag','SRFT','VerticalAlignment','top','Color','w','PickableParts','none');
   end
   ...
   if isequal(strcmp(get(vstm,'Checked'),'on'),1) % white background
   plot3(axesm,Evalue(:,1),-Evalue(:,3),Evalue(:,2),'Color',[0.4 0.4 0.4],'Tag',['SRF',num2str(i)]);
   hold on;
   elseif isequal(strcmp(get(vstm,'Checked'),'on'),0) % black background
   plot3(axesm,Evalue(:,1),-Evalue(:,3),Evalue(:,2),'Color','y','Tag',['SRF',num2str(i)]);
   hold on;       
   end
   Evalue = [];
