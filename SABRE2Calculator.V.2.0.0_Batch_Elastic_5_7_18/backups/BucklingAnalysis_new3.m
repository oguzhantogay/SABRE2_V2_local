function [gamma] = BucklingAnalysis_new(Geometry_data,Member_data,Loading_data,...
...
BCcount = sum(sum(BC_data(2:8,:)));
while increment < (ninc/lambda)+1
KtBB=zeros(nDOF);                                                           % initialize global tangent stiffness matrix
KtFF=zeros(nDOF-BCcount);                                                   % initialize global tangent stiffness matrix for free dof's
...
Rd(3,4)=offset_support(Member_data(2,i),1);           
...
[Kt]=StiffnessELEMENT3DESL_slender(Ae,yuc,Iux,Iy,Ip,Aft,Dt,...              % Does not currently handle releases, need to finish warping free??
    tfte,huft,Afb,tfbe,Ift,Ifb,Iwy,hufb,Ipp,Cwa,Db,L(j,1),E(i,1),wts(j,:),Rt,Rb,...
    theta1x,theta1z,theta1y,theta2x,theta2z,theta2y,theta1xp,theta2xp,...
    Aweb,hte,hbe,nip,xe(j,:),J,G(i,1));   

% Elastic Tangent Stiffness Matrix
Kts = Ge*Rd'*(Pbar'*Kt*Pbar)*Rd*Ge';

% Geometric Stiffness Matrix
Kgs=zeros(size(Kts,1),size(Kts,2));

[KtBB,KgBB] = assemble1(KtBB,KgBB,Kts,Kgs,Member_data,i,xn,Material_data);% Elastic stiffness & Geometric stiffness
...
[KtFF,KgFF] = partition(KtBB,KgBB,BC_data,Member_data,Material_data,KtFF);

% ------------------------------------------------------------------------
% -------------------       Report Final Results       -------------------
% ------------------------------------------------------------------------
[VV,tmp2]=eig(KtBB,-KgBB);                                                 % calculate eigen value
tmp2=diag(tmp2);
if max(tmp2(:,1)) <= 0
  [VV,tmp2]=eigs(KtBB,-KgBB,30,'SM');
  tmp2=diag(tmp2);
end
