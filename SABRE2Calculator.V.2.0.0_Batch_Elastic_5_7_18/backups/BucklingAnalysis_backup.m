function [gamma] = BucklingAnalysis(Geometry_data,Member_data,Loading_data,...
    DistLoading_data,BC_data,Material_data)    
% ************************************************************************
...
% ------------------------------------------------------------------------
[xg,yg,zg,bfb,tfb,bft,tft,Dg,tw,Dt,Db,ht,hb,hg1,hsb1,Dsb1,...
hst1,Dst1,Dmid1,Ag1,ytbar1,Dct1,Dcb1,hct1,hcb1,CSD1,Afillet]=deal(zeros...
(nmembers,1));                                                              % preallocate geometry arrays
for i=1:nnodes
    ...
    bfb(i,1)=Geometry_data(4,i);                                            % Bottom flange width start
    tfb(i,1)=Geometry_data(5,i);                                            % Bottom flange thickness start
    bft(i,1)=Geometry_data(6,i);                                            % Top flange width start
    tft(i,1)=Geometry_data(7,i);                                            % Top flange thickness start
    Dg(i,1)=Geometry_data(8,i);                                             % Web depth start
    tw(i,1)=Geometry_data(9,i);                                             % Web thickness start
    Afillet(i,1)=Geometry_data(9,i);                                        % Area of fillet weld
    ...
    % Shear center
    hg1(i,1)  = Dg(i,1)+tft(i,1)/2+tfb(i,1)/2;                           % Distance between flange centroids
    hsb1(i,1) = (tft(i,1).*bft(i,1).^3.*hg1(i,1))./(...
        tfb(i,1).*bfb(i,1).^3+tft(i,1).*bft(i,1).^3);                   % bottom flange centroid to shear center
    Dsb1(i,1) = hsb1(i,1) - tfb(i,1)/2;                                    % bottom of Web depth to shear center
    hst1(i,1) = hg1(i,1) - hsb1(i,1);                                       % top flange centroid to shear center
    Dst1(i,1) = hst1(i,1) - tft(i,1)/2;                                    % top of Web depth to shear center
    Dmid1(i,1)= Dsb1(i,1) - Dg(i,1)/2;                                     % web mid depth to shear center  
    hg2(i,1)  = Dg(i,1)+tft(i,1)/2+tfb(i,1)/2;                           % Distance between flange centroids
    hsb2(i,1) = (tft(i,1).*bft(i,1).^3.*hg2(i,1))./(...
        tfb(i,1).*bfb(i,1).^3+tft(i,1).*bft(i,1).^3);                   % bottom flange centroid to shear center
    Dsb2(i,1) = hsb2(i,1) - tfb(i,1)/2;                                    % bottom of Web depth to shear center
    hst2(i,1) = hg2(i,1) - hsb2(i,1);                                       % top flange centroid to shear center
    Dst2(i,1) = hst2(i,1) - tft(i,1)/2;                                    % top of Web depth to shear center
    Dmid2(i,1)= Dsb2(i,1) - Dg(i,1)/2;                                     % web mid depth to shear center

    % Centroid Axis
    Ag1(i,1) =  tft(i,1).*bft(i,1) + tw(i,1).*Dg(i,1)...
        + tfb(i,1).*bfb(i,1);                                             % gross area
    ytbar1(i,1) = (bft(i,1).*tft(i,1).*tft(i,1)/2+tw(i,1).*Dg(...
        i,1).*(tft(i,1)+Dg(i,1)/2)+ bfb(i,1).*tfb(i,1).*...             % top flange centroid to section centroid
        (tft(i,1)+Dg(i,1)+tfb(i,1)/2))./Ag1(i,1);
    Dct1(i,1) = ytbar1(i,1) - tft(i,1);                                    % top of Web depth to section centroid
    Dcb1(i,1) = Dg(i,1) - Dct1(i,1);                                       % bottom of Web depth to section centroid
    hct1(i,1) = ytbar1(i,1) - tft(i,1)/2;                                  % top flange centroid to section centroid
    hcb1(i,1) = hg1(i,1) - hct1(i,1);                                       % bottom flange centroid to section centroid
    Ag2(i,1) =  tft(i,1).*bft(i,1) + tw(i,1).*Dg(i,1)...
        + tfb(i,1).*bfb(i,1);                                             % gross area
    ytbar2(i,1) = ( bft(i,1).*tft(i,1).*tft(i,1)/2+tw(i,1).*Dg(...
        i,1).*(tft(i,1)+Dg(i,1)/2)+ bfb(i,1).*tfb(i,1).*...             % top flange centroid to section centroid
        (tft(i,1)+Dg(i,1)+tfb(i,1)/2) )./Ag2(i,1);
    Dct2(i,1) = ytbar2(i,1) - tft(i,1);                                    % top of Web depth to section centroid
    Dcb2(i,1) = Dg(i,1) - Dct2(i,1);                                       % bottom of Web depth to section centroid
    hct2(i,1) = ytbar2(i,1) - tft(i,1)/2;                                  % top flange centroid to section centroid
...
for i=1:nnodes
    ...
    elseif BC_data(1,i)==2                                                  % Top
        offset_support(i,1)=-hst1(i,1)-tft(i,1)/2;      
    elseif BC_data(1,i)==3                                                  % Bottom
        offset_support(i,1)=hsb1(i,1)+tfb(i,1)/2;
...
for i=1:nnodes   
    ...
    elseif Loading_data(7,i)==2                                             % Top
        offset_pointload(i,1)=-hst1(i,1)-tft(i,1)/2+Loading_data(8,i);      
    elseif Loading_data(7,i)==3                                             % Bottom
        offset_support(i,1)=hsb1(i,1)+tfb(i,1)/2+Loading_data(8,i);
...
for i=1:size(DistLoading_data,2) 
    ...
    elseif DistLoading_data(2,i)==2                                         % Top
        offset_distload(i,1)=-hst1(i,1)-tft(i,1)/2+Loading_data(8,Member_data(2,DistLoading_data(1,i)));      
    elseif DistLoading_data(2,i)==3                                         % Bottom
        offset_distload(i,1)=hsb1(i,1)+tfb(i,1)/2+Loading_data(8,Member_data(2,DistLoading_data(1,i)));
...
% %         elseif Loading_data(7,i)==2                                         % Top
% %          hd1(counter,1)=-hst1(i,1)-tft(i,1)/2+Loading_data(8,i);
% %         elseif Loading_data(7,i)==3                                         % Bottom
% %          hd1(counter,1)=-hsb1(i,1)+hb(i,1)+LNC1(i,12);
% %         elseif Loading_data(7,i)==4                                         % Centroid
...
% %          hd2(i,1)=hst2(i,1)+tft(i,1)/2 +LNC2(i,12);
% %         elseif isequal(LNC2(i,13),3) % Mid-Web
% %          hd2(i,1)=-hsb2(i,1)+hb(i,1)+LNC1(i,12);
...
Rd(3,4)=offset_support(Member_data(2,i),1);           %%%% should these be negative? I think it is taken care of in offset_support formulation
Rd(10,11)=offset_support(Member_data(3,i),1); 
for j=1:Material_data(1,i)
xn=xn+1;
% % Rd(1,6)=-offset_support(Member_data(2,i),1)*cos(-angletap(xn,2));         % Uniform load offset???
% % Rd(8,13)=-offset_support(Member_data(3,i),1)*cos(-angletap(xn,2));        % Uniform load offset???
% % Rd(2,6)=-offset_support(Member_data(2,i),1)*sin(-angletap(xn,2));         % Self-weight offset???
% % Rd(9,13)=-offset_support(Member_data(3,i),1)*sin(-angletap(xn,2));        % Self-weight offset???
...
% % Lsd(1,6)=CSD1(xn,1); Lsd(8,13)=CSD2(xn,1);                       
...
yse=Dmid1(Member_data(2,i),1)*(1-xe/L)+Dmid2(Member_data(3,i),1)*xe/L;
...
yse=Dmid1(Member_data(2,i),1);
end

% % hstp = ( hst2(i,1)-hst1(Member_data(2,i),1) )/DX(i,1);
% % hsbp = ( hsb2(i,1)-hsb1(Member_data(2,i),1) )/DX(i,1);      %%%%%%finish this
% % ysp  = ( ys2(i,1) -ys1(Member_data(2,i),1)  )/DX(i,1);
% % bftp = ( bft(i,1)-bft(Member_data(2,i),1) )/DX(i,1);
% % bfbp = ( bfb(i,1)-bfb(Member_data(2,i),1) )/DX(i,1);
