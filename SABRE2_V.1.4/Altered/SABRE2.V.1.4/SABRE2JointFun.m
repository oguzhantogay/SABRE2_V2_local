function SABRE2JointFun(pn_panel,pt_title_name,fim_infor_name,pd_type_subpanel,pdn_type_text,...
...
set([pdn_type_text,pdn_type_name],'Visible','on')
% *** sub panel2
set(pd_coord_subpanel,'Visible','on');
set([pdn_coord_text,pdn_coordx_text,pdn_coordy_text,pdn_coordz_text],'Visible','on');
set([pdn_coordx_edit,pdn_coordy_edit,pdn_coordz_edit,punit_text,punit_edit],'Visible','on');
% *** sub panel3
set(pd_app_subpanel,'Visible','on');
set([pdn_apply,pdn_cancel],'Visible','on'); 
...
% *************** If click Joint node, set the data automatically. 
if strcmp(get(gco,'type'),'line')
   if strcmp(sscanf(get(gco,'Tag'),'%[jn]'),'jn')
      njnode =sscanf(get(gco,'Tag'),'jn%u'); 
      set(findobj('Color','g'),'Color','r')
      set(gco,'Color','g')      
      set(pdn_type_name,'String',num2str(njnode));
      set(pdn_coordx_edit,'String',num2str(JNodevalue(njnode,2)));
      set(pdn_coordy_edit,'String',num2str(JNodevalue(njnode,3)));
      set(pdn_coordz_edit,'String',num2str(JNodevalue(njnode,4)));        
   end
end  
