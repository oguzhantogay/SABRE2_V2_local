function [ULoad,t]=SABRE2UnifFun(pn_panel,pt_title_name,fim_infor_name,pd_type_subpanel,pdn_type_text,...
...
% *** sub panel2
set([pam_segmentn_text,pamse_assign_text],'Visible','on');
...
% *** sub panel1
set(lum_type_text,'Visible','on')
% *** sub panel2
set([lum_type_mem,lum_type_seg],'Visible','on');
set(lum_clc_apply,'Visible','on');
% *** sub panel3
set(lum_height_edit,'Visible','on');
% *** sub panel4
set([lum_wx_text,lum_wy_text,lum_wz_text],'Visible','on');
set([lum_wx_edit,lum_wy_edit,lum_wz_edit],'Visible','on');
% *** sub panel5
set([lum_apply,lum_cancel],'Visible','on'); 
...
if isempty(SNodevalue) || ~isequal(error,0)
   ...
   set(findobj('Tag','S'),'Visible','off');
else
   set(pt_title_name,'Visible','off') 
   set(findobj('Tag','S'),'Visible','on');
   set(findobj('Tag','M'),'Visible','off');
end
...
if strcmp(get(gco,'type'),'text') 
   if strcmp(get(gco,'Tag'),'S')            
      ...
      mnum = t(1,1);
      snum = t(2,1);
      set(lum_type_mem,'String',num2str(mnum));
      set(lum_type_seg,'String',num2str(snum)); 
      ...
      for i=1:max(SNodevalue(:,1,1))
         ...
         for j=1:max(SNodevalue(i,:,2))
            ...
            if isequal(mnum,i) && isequal(snum,j)
               for k=1:SNodevalue(i,j,3)
%                      [1+p+q,max(SNodevalue(i,j,3))+p+q]
                  ULoad1(k,1)=DUP1(k+p+q,2);
                  ULoad1(k,2)=DUP2(k+p+q,2);
               end
            end
            p=p+SNodevalue(i,j,3);
         end  
         q=q+p;
      end
      ULoad=unique(ULoad1);
      ...
      if ~isempty(ULoad) 
         if ~isempty(LUEC) 
            if min(ULoad(:,1)) < max(LUEC(:,1))
               for i=1:length(LUEC(:,1))
                  if isequal(LUEC(i,2),ULoad(round(length(ULoad(:,1))/2),1))
                     dnum=i;
                  end

               end                     
               set(lum_wx_edit,'string',LUEC( dnum ,5));
               set(lum_wy_edit,'string',LUEC( dnum ,6));      
               set(lum_wz_edit,'string',LUEC( dnum ,7)); 
               if isequal(LUEC( dnum ,17),2) || isequal(LUEC( dnum ,17),3) || isequal(LUEC( dnum ,17),4) || isequal(LUEC( dnum ,17),5)
                  set(lum_height_edit,'Value',LUEC( dnum ,17));
               else
                  set(lum_height_edit,'Value',1);
               end
