function    [RNCc,Nshe1,Nshe2,DUP1,DUP2,NCc,Bhe1,Bhe2,The1,The2,SGhe1,SGhe2,Mhe1,Mhe2,Bhg,Thg,CSg,Mhg,BNC1,BNC2,FEL,error,LNC,LNC1,LNC2,LUEC,PNC,PNC1,PNC2,BNC]=SABRE2MLCODEW(JNodevalue,...
   Massemble,JNodevalue_i,JNodevalue_j,Rval,BNodevalue,SNodevalue,...
   RNCc,Nshe1,Nshe2,DUP1,DUP2,NCc,LNCp,LNCp1,LNCp2,LUECp,PNCp,PNCp1,PNCp2,BNCp,BNCp1,BNCp2,FELp,Bhe1,Bhe2,The1,The2,SGhe1,SGhe2,Mhe1,Mhe2,Bhg,Thg,CSg,Mhg,pdmi_name,pdmi_type_edit,LabType,pt_title_name,axesm,vstm)
...
% ------                            Mirror                        --------
% ------------------------------------------------------------------------    
   LNC=[]; LNC1=[]; LNC2=[]; LUEC=[]; PNC=[]; PNC1=[]; PNC2=[]; BNC=[]; BNC1=[]; BNC2=[];FEL=[];
   if isequal(get(pdmi_type_edit,'Value'),1)
      % LNC,LNC1,LNC2
      if isempty(LNCp)
         LNC=[]; LNC1=[]; LNC2=[];
      else	
         ...
         for i=1:length(LNCp(:,1))
            LNC(i,:)= LNCp(i,:); 
         end
         
         for i=1:length(LNCp1(:,1))
            LNC1(i,:)= LNCp1(i,:);
            LNC2(i,:)= LNCp2(i,:);          
         end        
      end
      ...
      if isempty(PNCp)
         ...
         for i=1:length(PNCp(:,1))
            PNC(i,:)= PNCp(i,:);
         end
         
         for i=1:length(PNCp1(:,1))
            PNC1(i,:)= PNCp1(i,:);
            PNC2(i,:)= PNCp2(i,:);            
         end        
      end        
      ...
      if isempty(LUECp)
         ...
         LUEC(length(DUP1(:,1)),20)=0; % zeros matrix length(NC(:,1)) x 11
         for i=1:length(LUECp(:,1))
           LUEC(i,:)= LUECp(i,:); 
         end         
      end
      % BNC
      if isempty(BNCp)
         ...
         BNC(length(RNCc(:,1)),14)=0;       
         for i=1:length(BNCp(:,1))
            BNC(i,:)= BNCp(i,:);
         end     
      end     
      % BNC1;BNC2
      ...
      if isempty(FELp)
         ...
         FEL(length(DUP1(:,1)),9)=0; % zeros matrix length(NC(:,1)) x 11
         for i=1:length(FELp(:,1))
           FEL(i,:)= FELp(i,:); 
         end         
      end      

   elseif isequal(get(pdmi_type_edit,'Value'),2)
      % LNC,LNC1,LNC2
      if isempty(LNCp)
         LNC=[]; LNC1=[]; LNC2=[];
      else	
         ...
         for i=1:length(LNCp(:,1))
            LNC(i,:)= LNCp(i,:);
            for j=(length(LNCp(:,1))+1):length(RNCc(:,1))
               if isequal(round((RNCc(i,2)-JNodevalue(mir,2))*10^6)/10^6,round((JNodevalue(mir,2)-RNCc(j,2))*10^6)/10^6)
                  if isequal(round(RNCc(i,3)*10^6)/10^6,round(RNCc(j,3)*10^6)/10^6)
                     LNC(j,1)= j;
                     LNC(j,2)= RNCc(j,2);
                     LNC(j,3)= RNCc(j,3);
                     LNC(j,4)= RNCc(j,4);
                     ...
                     LNC(j,11)= LNCp(i,11);
                     LNC(j,12)= LNCp(i,12);
                     if isequal(LNCp(i,13),2)
                        LNC(j,13)= 3;
                     elseif isequal(LNCp(i,13),3) 
                        LNC(j,13)= 2;
                     else
                        LNC(j,13)= LNCp(i,13);
                     end  
         ...
         for i=1:length(LNCp1(:,1))
            LNC1(i,:)= LNCp1(i,:);
            LNC2(i,:)= LNCp2(i,:);           
            for j=(length(LNCp1(:,1))+1):length(DUP1(:,1))
               if isequal(round((RNCc(DUP1(i,2),2)-JNodevalue(mir,2))*10^6)/10^6,round((JNodevalue(mir,2)-RNCc(DUP1(j,2),2))*10^6)/10^6)
                  if isequal(round(RNCc(DUP1(i,2),3)*10^6)/10^6,round(RNCc(DUP1(j,2),3)*10^6)/10^6)
                     LNC1(j,1)= DUP1(j,2);
                     LNC1(j,2)= RNCc(DUP1(j,2),2);
                     LNC1(j,3)= RNCc(DUP1(j,2),3);
                     LNC1(j,4)= RNCc(DUP1(j,2),4);
                     LNC1(j,5)= -LNCp1(i,5);
                     LNC1(j,6)= LNCp1(i,6);
                     LNC1(j,7)= LNCp1(i,7);
                     LNC1(j,8)= LNCp1(i,8);
                     LNC1(j,9)= LNCp1(i,9);
                     LNC1(j,10)= -LNCp1(i,10);
                     LNC1(j,11)= LNCp1(i,11);
                     LNC1(j,12)= LNCp1(i,12);
                     if isequal(LNCp1(i,13),2)
                        LNC1(j,13)= 3;
                     elseif isequal(LNCp1(i,13),3) 
                        LNC1(j,13)= 2;
                     else
                        LNC1(j,13)= LNCp1(i,13);
                     end  
                     LNC1(j,14)= LNCp1(i,14);                    
                  end
               end
               
               if isequal(round((RNCc(DUP2(i,2),2)-JNodevalue(mir,2))*10^6)/10^6,round((JNodevalue(mir,2)-RNCc(DUP2(j,2),2))*10^6)/10^6)
                  if isequal(round(RNCc(DUP2(i,2),3)*10^6)/10^6,round(RNCc(DUP2(j,2),3)*10^6)/10^6)
                     LNC2(j,1)= DUP2(j,2);
                     LNC2(j,2)= RNCc(DUP2(j,2),2);
                     LNC2(j,3)= RNCc(DUP2(j,2),3);
                     LNC2(j,4)= RNCc(DUP2(j,2),4);
                     LNC2(j,5)= -LNCp2(i,5);
                     LNC2(j,6)= LNCp2(i,6);
                     LNC2(j,7)= LNCp2(i,7);
                     LNC2(j,8)= LNCp2(i,8);
                     LNC2(j,9)= LNCp2(i,9);
                     LNC2(j,10)= -LNCp2(i,10);
                     LNC2(j,11)= LNCp2(i,11);
                     LNC2(j,12)= LNCp2(i,12);
                     if isequal(LNCp2(i,13),2)
                        LNC2(j,13)= 3;
                     elseif isequal(LNCp2(i,13),3) 
                        LNC2(j,13)= 2;
                     else
                        LNC2(j,13)= LNCp2(i,13);
                     end  
                     LNC2(j,14)= LNCp2(i,14);
                  end
               end               
            end            
         end            
      ...
      if isempty(PNCp)
         ...
         for i=1:length(PNCp(:,1))
            PNC(i,:)= PNCp(i,:);
            for j=(length(PNCp(:,1))+1):length(RNCc(:,1))
%                [j,JNodevalue(mir,2),RNCc(i,2)-JNodevalue(mir,2),JNodevalue(mir,2)-RNCc(j,2)]
               if isequal(round((RNCc(i,2)-JNodevalue(mir,2))*10^6)/10^6,round((JNodevalue(mir,2)-RNCc(j,2))*10^6)/10^6)
                  if isequal(round(RNCc(i,3)*10^6)/10^6,round(RNCc(j,3)*10^6)/10^6)
                     PNC(j,1)= j;
                     PNC(j,2)= RNCc(j,2);
                     ...
                     PNC(j,11)= PNCp(i,11);
                     PNC(j,12)= PNCp(i,12);
                     if isequal(PNCp(i,13),2)
                        PNC(j,13)= 3;
                     elseif isequal(PNCp(i,13),3) 
                        PNC(j,13)= 2;
                     else
                        PNC(j,13)= PNCp(i,13);
                     end  
         ...
         for i=1:length(PNCp1(:,1))
            PNC1(i,:)= PNCp1(i,:);
            PNC2(i,:)= PNCp2(i,:);            
            for j=(length(PNCp1(:,1))+1):length(DUP1(:,1))
               if isequal(round((RNCc(DUP1(i,2),2)-JNodevalue(mir,2))*10^6)/10^6,round((JNodevalue(mir,2)-RNCc(DUP1(j,2),2))*10^6)/10^6)
                  if isequal(round(RNCc(DUP1(i,2),3)*10^6)/10^6,round(RNCc(DUP1(j,2),3)*10^6)/10^6)                    
                     PNC1(j,1)= DUP1(j,2);
                     PNC1(j,2)= RNCc(DUP1(j,2),2);
                     PNC1(j,3)= RNCc(DUP1(j,2),3);
                     PNC1(j,4)= RNCc(DUP1(j,2),4);
                     PNC1(j,5)= PNCp1(i,5);
                     PNC1(j,6)= PNCp1(i,6);
                     PNC1(j,7)= PNCp1(i,7);
                     PNC1(j,8)= PNCp1(i,8);
                     PNC1(j,9)= PNCp1(i,9);
                     PNC1(j,10)= PNCp1(i,10);
                     PNC1(j,11)= PNCp1(i,11);
                     PNC1(j,12)= PNCp1(i,12);
                     if isequal(PNCp1(i,13),2)
                        PNC1(j,13)= 3;
                     elseif isequal(PNCp1(i,13),3) 
                        PNC1(j,13)= 2;
                     else
                        PNC1(j,13)= PNCp1(i,13);
                     end                        
                     PNC1(j,14)= PNCp1(i,14);                     
                  end
               end               
               if isequal(round((RNCc(DUP2(i,2),2)-JNodevalue(mir,2))*10^6)/10^6,round((JNodevalue(mir,2)-RNCc(DUP2(j,2),2))*10^6)/10^6)
                  if isequal(round(RNCc(DUP2(i,2),3)*10^6)/10^6,round(RNCc(DUP2(j,2),3)*10^6)/10^6)
                     PNC2(j,1)= DUP2(j,2);
                     PNC2(j,2)= RNCc(DUP2(j,2),2);
                     PNC2(j,3)= RNCc(DUP2(j,2),3);
                     PNC2(j,4)= RNCc(DUP2(j,2),4);
                     PNC2(j,5)= PNCp2(i,5);
                     PNC2(j,6)= PNCp2(i,6);
                     PNC2(j,7)= PNCp2(i,7);
                     PNC2(j,8)= PNCp2(i,8);
                     PNC2(j,9)= PNCp2(i,9);
                     PNC2(j,10)= PNCp2(i,10);
                     PNC2(j,11)= PNCp2(i,11);
                     PNC2(j,12)= PNCp2(i,12);
                     if isequal(PNCp2(i,13),2)
                        PNC2(j,13)= 3;
                     elseif isequal(PNCp2(i,13),3) 
                        PNC2(j,13)= 2;
                     else
                        PNC2(j,13)= PNCp2(i,13);
                     end                     
                     PNC2(j,14)= PNCp2(i,14);
                  end
               end               
            end            
         end            
      ...
      if isempty(LUECp)
         ...
         for i=1:length(LUECp(:,1))           
            LUEC(i,:)= LUECp(i,:); 
            for j=(length(LUECp(:,1))+1):length(DUP1(:,1))
               if isequal(round((RNCc(DUP1(i,2),2)-JNodevalue(mir,2))*10^6)/10^6,round((JNodevalue(mir,2)-RNCc(DUP1(j,2),2))*10^6)/10^6) ...
                     && isequal(round((RNCc(DUP2(i,2),2)-JNodevalue(mir,2))*10^6)/10^6,round((JNodevalue(mir,2)-RNCc(DUP2(j,2),2))*10^6)/10^6)               
                  if isequal(round(RNCc(DUP1(i,2),3)*10^6)/10^6,round(RNCc(DUP1(j,2),3)*10^6)/10^6) ...
                        && isequal(round(RNCc(DUP2(i,2),3)*10^6)/10^6,round(RNCc(DUP2(j,2),3)*10^6)/10^6) 
                     LUEC(j,1)= DUP1(j,1);
                     LUEC(j,2)= DUP1(j,2);
                     LUEC(j,3)= DUP2(j,2);
                     LUEC(j,4)= 0;
                     ...
                     LUEC(j,8)= LUECp(i,8);
                     LUEC(j,9)= LUECp(i,9);
                     LUEC(j,10)= LUECp(i,10);
                     LUEC(j,11)= LUECp(i,11);
                     LUEC(j,12)= LUECp(i,12);
                     LUEC(j,13)= LUECp(i,13);
                     LUEC(j,14)= LUECp(i,14);
                     LUEC(j,15)= LUECp(i,15);
                     LUEC(j,16)= LUECp(i,16);
                     if isequal(LUECp(i,17),2)
                        LUEC(j,17)= 3;
                     elseif isequal(LUECp(i,17),3) 
                        LUEC(j,17)= 2;
                     else
                        LUEC(j,17)= LUECp(i,17);
                     end
                     LUEC(j,18)= LUECp(i,18);
                     LUEC(j,19)= LUECp(i,19);
                  end
               end                             
            end                       
         end   
      ...
      if isempty(BNCp)
         ...
         for i=1:length(BNCp(:,1))
            BNC(i,:)= BNCp(i,:);
            for j=(length(BNCp(:,1))+1):length(RNCc(:,1))
               if isequal(round((RNCc(i,2)-JNodevalue(mir,2))*10^6)/10^6,round((JNodevalue(mir,2)-RNCc(j,2))*10^6)/10^6)
                  if isequal(round(RNCc(i,3)*10^6)/10^6,round(RNCc(j,3)*10^6)/10^6)
                     BNC(j,1)= j;
                     BNC(j,2)= RNCc(j,2);
                     ...
                     BNC(j,11)= BNCp(i,11);
                     BNC(j,12)= BNCp(i,12);
                     if isequal(BNCp(i,13),2)
                        BNC(j,13)= 3;
                     elseif isequal(BNCp(i,13),3) 
                        BNC(j,13)= 2;
                     else
                        BNC(j,13)= BNCp(i,13);
                     end  
      ...
      if isempty(BNCp1)
         ...
         sbcnum1 = length(BNCp1(:,1))+1;
         sbcnum2 = length(BNCp1(:,1))+1;
         for i=1:length(BNCp1(:,1))
               BNC1(i,:)= BNCp1(i,:);
               BNC2(i,:)= BNCp2(i,:);  
         end         
         for i=1:length(BNCp1(:,1))      
               for j=1:length(DUP1(:,1))
                  if ~isequal(round(RNCc(BNC1(i,2),2)*10^6)/10^6,round(RNCc(DUP1(j,2),2)*10^6)/10^6) 
                     if isequal(round((RNCc(BNC1(i,2),2)-JNodevalue(mir,2))*10^6)/10^6,round((JNodevalue(mir,2)-RNCc(DUP1(j,2),2))*10^6)/10^6)               
                        if isequal(round(RNCc(BNC1(i,2),3)*10^6)/10^6,round(RNCc(DUP1(j,2),3)*10^6)/10^6) 
                           inode=DUP1(j,2);
                           BNC1(sbcnum1,1) =  sbcnum1;
                           BNC1(sbcnum1,2) = inode;
                           BNC1(sbcnum1,3) = RNCc(inode,2);
                           BNC1(sbcnum1,4) = RNCc(inode,3);
                           BNC1(sbcnum1,5) = RNCc(inode,4);
                           BNC1(sbcnum1,6)=0;
                           BNC1(sbcnum1,7)=0;
                           BNC1(sbcnum1,8)=BNCp1(i,8);
                           if isequal(BNCp1(i,9),1)
                              BNC1(sbcnum1,9)=3;
                           elseif isequal(BNCp1(i,9),3)
                              BNC1(sbcnum1,9)=1;
                           else
                              BNC1(sbcnum1,9)=BNCp1(i,9);
                           end
                           BNC1(sbcnum1,10)=BNCp1(i,10)+max(BNCp1(:,10)); 
                           sbcnum1=sbcnum1+1;
                        end
                     end                   
                  end

                   if ~isequal(round(RNCc(BNC2(i,2),2)*10^6)/10^6,round(RNCc(DUP2(j,2),2)*10^6)/10^6) 
                     if isequal(round((RNCc(BNC2(i,2),2)-JNodevalue(mir,2))*10^6)/10^6,round((JNodevalue(mir,2)-RNCc(DUP2(j,2),2))*10^6)/10^6)               
                        if isequal(round(RNCc(BNC2(i,2),3)*10^6)/10^6,round(RNCc(DUP2(j,2),3)*10^6)/10^6) 
                           jnode=DUP2(j,2);
                           BNC2(sbcnum2,1) =  sbcnum2;
                           BNC2(sbcnum2,2) = jnode;
                           BNC2(sbcnum2,3) = RNCc(jnode,2);
                           BNC2(sbcnum2,4) = RNCc(jnode,3);
                           BNC2(sbcnum2,5) = RNCc(jnode,4);
                           BNC2(sbcnum2,6)=0;
                           BNC2(sbcnum2,7)=0;
                           BNC2(sbcnum2,8)=BNCp2(i,8);
                           if isequal(BNCp2(i,9),1)
                              BNC2(sbcnum2,9)=3;
                           elseif isequal(BNCp2(i,9),3)
                              BNC2(sbcnum2,9)=1;
                           else
                              BNC2(sbcnum2,9)=BNCp2(i,9);
                           end
                           BNC2(sbcnum2,10)=BNCp2(i,10)+max(BNCp2(:,10));                           
                           sbcnum2=sbcnum2+1;                           
                        end
                     end                   
                  end                 
      ...
      if isempty(FELp)
         ...
         for i=1:length(FELp(:,1))
           FEL(i,:)= FELp(i,:); 
            ...
            for j=(length(FELp(:,1))+1):length(DUP1(:,1))
               if isequal(round((RNCc(DUP1(i,2),2)-JNodevalue(mir,2))*10^6)/10^6,round((JNodevalue(mir,2)-RNCc(DUP1(j,2),2))*10^6)/10^6) ...
                     && isequal(round((RNCc(DUP2(i,2),2)-JNodevalue(mir,2))*10^6)/10^6,round((JNodevalue(mir,2)-RNCc(DUP2(j,2),2))*10^6)/10^6)               
                  if isequal(round(RNCc(DUP1(i,2),3)*10^6)/10^6,round(RNCc(DUP1(j,2),3)*10^6)/10^6) ...
                        && isequal(round(RNCc(DUP2(i,2),3)*10^6)/10^6,round(RNCc(DUP2(j,2),3)*10^6)/10^6)                     
...
%   PNC=[]; PNC1=[]; PNC2=[]; BNC=[]; BNC1=[]; BNC2=[];FEL=[];
%    
   if ~isempty(LNC)
      LNCmax = max(max(abs(LNC(:,5:10))));
      if isequal(LNCmax,0)
         LNC=[];LNC1=[];LNC2=[];
      end 
   end
   ...
   if ~isempty(LUEC)
      LUECmax = max(max(abs(LUEC(:,5:7))));
      if isequal(LUECmax,0)
         LUEC=[];
      end 
   end   
   
   if ~isempty(PNC)
      PNCmax = max(max(abs(PNC(:,5:11))));
      if isequal(PNCmax,0)
         PNC=[];PNC1=[];PNC2=[];
      end 
   end
   ...
   if ~isempty(BNC1)
      BNC1max = max(abs(BNC1(:,8)));
      if isequal(BNC1max,0)
         BNC1=[];BNC2=[];
      end 
   end   
%    mir =str2double(get(pdmi_name,'String'));
   ...
   if isempty(RNCc) || isempty(LNC)
      ...
      for i=1:length(RNCc(:,1))
            ...
            elseif isequal(LNC(i,13),5) % Mid-Web
               if LNC(i,5) < 0
                  plot3(axesm,[(RNCc(i,2)+dp1+Mhg(i,1)), (RNCc(i,2)+dp2+Mhg(i,1))],...
                     [RNCc(i,4)+Mhg(i,3),RNCc(i,4)+Mhg(i,3)],[RNCc(i,3)+Mhg(i,2),RNCc(i,3)+Mhg(i,2)],'HitTest','off','Color','k',...
                     'Tag',['LNC',num2str(i)],'linewidth',lt,'PickableParts','none');
                  hold on;                  
            ...
            elseif isequal(LNC(i,13),5) % Mid-Web 
               if LNC(i,6) < 0
                  plot3(axesm,[RNCc(i,2)+Mhg(i,1),RNCc(i,2)+Mhg(i,1) ],[RNCc(i,4)+Mhg(i,3),RNCc(i,4)+Mhg(i,3)],...
                     [(RNCc(i,3)+dp1+Mhg(i,2)),(RNCc(i,3)+dp2+Mhg(i,2))],'HitTest','off','Color','k',...
                     'Tag',['LNC',num2str(i)],'linewidth',lt,'PickableParts','none');
                  hold on;                  
            ...
            elseif isequal(LNC(i,13),5) % Mid-Web        
               if LNC(i,7) > 0
                  plot3(axesm,[RNCc(i,2)+Mhg(i,1),RNCc(i,2)+Mhg(i,1) ],[(RNCc(i,4)+dp1+Mhg(i,3)),(RNCc(i,4)+dp2+Mhg(i,3))],...
                     [RNCc(i,3)+Mhg(i,2),RNCc(i,3)+Mhg(i,2)],'HitTest','off','Color','k',...
                     'Tag',['LNC',num2str(i)],'linewidth',lt,'PickableParts','none');
                  hold on; 
         ...
         if ~isequal(LNC(i,8),0) % Torsion
            ...
            elseif isequal(LNC(i,13),5) % Mid-Web
                plot3(axesm,z+RNCc(i,2)+Mhg(i,1),x+RNCc(i,4)+Mhg(i,3),y+RNCc(i,3)+Mhg(i,2),'HitTest','off','Color','k',...
                  'Tag',['LNC',num2str(i)],'linewidth',lt,'PickableParts','none');
               hold on;
               if LNC(i,8) > 0
         ...
         if ~isequal(LNC(i,9),0) % Moment y-axis
            ...
            elseif isequal(LNC(i,13),5) % Mid-Web
               if LNC(i,9) > 0
                  plot3(axesm,x+RNCc(i,2)+Mhg(i,1),y+RNCc(i,4)+Mhg(i,3),z+RNCc(i,3)+Mhg(i,2),'HitTest','off','Color','k',...
                     'Tag',['LNC',num2str(i)],'linewidth',lt,'PickableParts','none');
                  hold on;                  
                  Sco = [RNCc(i,2)+Mhg(i,1),RNCc(i,4)-prd+Mhg(i,3),RNCc(i,3)+Mhg(i,2); RNCc(i,2)+Mhg(i,1),RNCc(i,4)-prd+Mhg(i,3),RNCc(i,3)+Mhg(i,2); ...
                     RNCc(i,2)+Mhg(i,1),RNCc(i,4)-prd+Mhg(i,3),RNCc(i,3)+Mhg(i,2); RNCc(i,2)+Mhg(i,1),RNCc(i,4)-prd+Mhg(i,3),RNCc(i,3)+Mhg(i,2)];
                  ...
                  hold on;                     
               elseif LNC(i,9) < 0
                  plot3(axesm,x1+RNCc(i,2)+Mhg(i,1),y1+RNCc(i,4)+Mhg(i,3),z+RNCc(i,3)+Mhg(i,2),'HitTest','off','Color','k',...
                     'Tag',['LNC',num2str(i)],'linewidth',lt,'PickableParts','none');
                  hold on;                   
                  Sco = [RNCc(i,2)+Mhg(i,1),RNCc(i,4)-prd+Mhg(i,3),RNCc(i,3)+Mhg(i,2); RNCc(i,2)+Mhg(i,1),RNCc(i,4)-prd+Mhg(i,3),RNCc(i,3)+Mhg(i,2); ...
                     RNCc(i,2)+Mhg(i,1),RNCc(i,4)-prd+Mhg(i,3),RNCc(i,3)+Mhg(i,2); RNCc(i,2)+Mhg(i,1),RNCc(i,4)-prd+Mhg(i,3),RNCc(i,3)+Mhg(i,2)];
         ...
         if ~isequal(LNC(i,10),0) % Moment z-axis
            ...
            elseif isequal(LNC(i,13),5) % Mid-Web
               if LNC(i,10) > 0
                  plot3(axesm,x1+RNCc(i,2)+Mhg(i,1),z+RNCc(i,4)+Mhg(i,3),y1+RNCc(i,3)+Mhg(i,2),'HitTest','off','Color','k',...
                     'Tag',['LNC',num2str(i)],'linewidth',lt,'PickableParts','none');
                  hold on;                    
                  ...
                  hold on; 
               elseif LNC(i,10) < 0
                  plot3(axesm,x+RNCc(i,2)+Mhg(i,1),z+RNCc(i,4)+Mhg(i,3),y+RNCc(i,3)+Mhg(i,2),'HitTest','off','Color','k',...
                     'Tag',['LNC',num2str(i)],'linewidth',lt,'PickableParts','none');
                  hold on;                  
                  Sco = [RNCc(i,2)+prd+Mhg(i,1),RNCc(i,4)+Mhg(i,3),RNCc(i,3)+Mhg(i,2); RNCc(i,2)+prd+Mhg(i,1),RNCc(i,4)+Mhg(i,3),RNCc(i,3)+Mhg(i,2); ...
                     RNCc(i,2)+prd+Mhg(i,1),RNCc(i,4)+Mhg(i,3),RNCc(i,3)+Mhg(i,2); RNCc(i,2)+prd+Mhg(i,1),RNCc(i,4)+Mhg(i,3),RNCc(i,3)+Mhg(i,2)];
