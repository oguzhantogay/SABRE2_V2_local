function SABRE2DiagramNone(JNodevalue,Massemble,SNodevalue,RNCc,NCc,...
   Nshe1,Nshe2,DUP1,DUP2,Rval,pt_title_name,LabType,axesm,vstm)
...
for i = 1:xn 
   ...
   if isequal(strcmp(get(vstm,'Checked'),'on'),1) % white background
     plot3(axesm,Evalue(:,1),-Evalue(:,3),Evalue(:,2),'Color',[0.4 0.4 0.4],'Tag',...
      'Nshe');
     hold on;
   elseif isequal(strcmp(get(vstm,'Checked'),'on'),0) % black background
      ...
      for i = 1:length(NCc(:,1))
         if isequal(strcmp(get(vstm,'Checked'),'on'),1) % white background
            plot3(axesm,NCc(i,2),-NCc(i,4),NCc(i,3),...
               'Marker','.','Color',[0 0.5 1],'MarkerSize',10,'Tag','DiagramFrame');
            hold on;
         elseif isequal(strcmp(get(vstm,'Checked'),'on'),0) % black background
            plot3(axesm,NCc(i,2),-NCc(i,4),NCc(i,3),...
               'Marker','.','Color','w','MarkerSize',10,'Tag','DiagramFrame');
            hold on;
         end
      end 
   end
