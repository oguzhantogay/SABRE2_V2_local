function [AS,Funew,Qintf,QintfE,Qintg,Qint1,Qint2,MI,Sincre,PrPeL2nd] = Analysis2nd(Massemble,Rval,...
...
PrPeL2nd=zeros(5,xn,(round(ninc/lambda))); % Preallocationg
...
for n = 1:xn
   ...
   DE=zeros(14,1); SE=zeros(14,1);
   if ~isempty(LUEC)
      [Kt,Kgg,Qino,Kh2,Gn]=StiffnessELEMENT3DDM([bfb1(n);bfb2(n)],...
         [tfb1(n);tfb2(n)],[bft1(n);bft2(n)],[tft1(n);tft2(n)],...
         [Dg1(n);Dg2(n)],[yc1(n);yc2(n)],[ht1(n);ht2(n)],[hb1(n);hb2(n)], ...
         [Dt1(n);Dt2(n)],[Db1(n);Db2(n)],[hst1(n);hst2(n)],[hsb1(n);hsb2(n)], ...
         [ys1(n);ys2(n)],[tw1(n);tw2(n)],[Afillet1(n),Afillet2(n)],L0(n,1),E(n,1),G(n,1),fillet(n,:),...
         hstp(n),hsbp(n),ysp(n),bftp(n),bfbp(n), ...
         theta1xn,theta1zn,theta1yn,theta2xn,theta2zn,theta2yn,...
         theta1xpn,theta2xpn,qno,LUNCe(n,2),LUEC(n,17),FEL(n,:),ap_da_buttongroup,taub(:,n));
      ...
      [Kt,Kgg,Qino,Kh2,Gn]=StiffnessELEMENT3DNDM([bfb1(n);bfb2(n)],...
         [tfb1(n);tfb2(n)],[bft1(n);bft2(n)],[tft1(n);tft2(n)],...
         [Dg1(n);Dg2(n)],[yc1(n);yc2(n)],[ht1(n);ht2(n)],[hb1(n);hb2(n)], ...
         [Dt1(n);Dt2(n)],[Db1(n);Db2(n)],[hst1(n);hst2(n)],[hsb1(n);hsb2(n)], ...
         [ys1(n);ys2(n)],[tw1(n);tw2(n)],[Afillet1(n),Afillet2(n)],L0(n,1),E(n,1),G(n,1),fillet(n,:),...
         hstp(n),hsbp(n),ysp(n),bftp(n),bfbp(n), ...
         theta1xn,theta1zn,theta1yn,theta2xn,theta2zn,theta2yn,...
         theta1xpn,theta2xpn,qno,FEL(n,:),ap_da_buttongroup,taub(:,n));   
   end
...
%       Rsd(3,4)=rd1(n);  Rsd(10,11)=rd2(n); 
      ...
      [GDetc]=CoordTransform(et1,et2,et3); % For 2nd-order
...
%    norm = sqrt(R'*R);
   ...
   if norm > 0.01      
      for i = 1:xn
          for j = 1:5
                  PrPeL2nd(j,i,increment) = PrPeL(j,i)*0;
          end
      end   
   else
      for i = 1:xn
          for j = 1:5
                  PrPeL2nd(j,i,increment) = PrPeL(j,i);
          end
      end         
   end
   ...
   if norm > 0.01      
      for i = 1:xn
          for j = 1:nodedof
                  Qint1(i,j,increment) = Qing(i,j)*0;
                  Qint2(i,j,increment) = Qing(i,j+nodedof)*0;
          end
      end   
   else
      for i = 1:xn
   ...
   for i=1:length(RNCc(:,1))
      for j=1:length(MI(:,1))
         if isequal(i,MI(j,2))  
            ...
            Qintg(i,4,increment)=Qint1(MI(j,1),4,increment);
            Qintg(i,5,increment)=Qint1(MI(j,1),5,increment);
            Qintg(i,6,increment)=Qint1(MI(j,1),6,increment);
            Qintg(i,7,increment)=Qint1(MI(j,1),7,increment);
         elseif isequal(i,MI(j,3))
            Qintg(i,1,increment)=Qint2(MI(j,1),1,increment);
            Qintg(i,2,increment)=Qint2(MI(j,1),2,increment);
            Qintg(i,3,increment)=Qint2(MI(j,1),3,increment);
            Qintg(i,4,increment)=Qint2(MI(j,1),4,increment);
            Qintg(i,5,increment)=Qint2(MI(j,1),5,increment);
            Qintg(i,6,increment)=Qint2(MI(j,1),6,increment);
   ...
   if norm > 0.01
      for i = 1:xn
          for j = 1:nodedof*2
                  QintfE(i,j,increment) = Qin(i,j)*0;
                  Qintf(i,j,increment) = Qin(i,j)*0;
          end
      end        
   else
      for i = 1:xn
...
if isequal(Bflag,1)
   delete(wh)
   set(pt_title_name,'String','2nd Order Elastic Analysis did not finish. Please try to check loads and boundry conditions.')
   ...
   waitbar(perc/((ninc/lambda)+1),wh,sprintf('%d%% along...',perc))
   delete(wh)   
   set(pt_title_name,'String','2nd Order Elatic Analysis is completed.')
   set(pt_title_name,'Visible','on')
   if norm > 0.01
      set(pt_title_name,'String','2nd Order Elastic Analysis did not finish. Please try to check loads and boundry conditions.')
      set(pt_title_name,'Visible','on')
