function [ANI,gammma,FunewR,EGunew,Qintf,QintfE,Qint1,Qint2,MI,tau,Rpg,Rpc,Rpt,Rh,Myc,Myt,My,Jval,Phi_Mmax,Phi_Py,UC,PrPeL,crLTB,LGv] = AnalysisNonIEBucklingCheck(Massemble,...
...
elseif isequal(Bflag,2)
   ...
   if max_UC >= 0.9999
      ...
      Qint1=Qint1(:,:,end);
      Qint2=Qint2(:,:,end);
      set(pt_title_name,'String',['Inelastic Nonlinear Buckling Check, Eigenvalue = ',num2str(gammma),', Maximum Cross-Section Unity Check = ',num2str(max_UC),'. The Structure Cannot Support the Specified Load.'])
      set(pt_title_name,'Visible','on')            
   else 
       Qintf=Qintf(:,:,end);
       QintfE=QintfE(:,:,end);
       Qint1=Qint1(:,:,end);
       Qint2=Qint2(:,:,end);
       set(pt_title_name,'String',['Inelastic Nonlinear Buckling Check, Eigenvalue = ',num2str(gammma),', Maximum Cross-Section Unity Check = ',num2str(max_UC),'. The Structure Can Support the Specified Load.'])   
       set(pt_title_name,'Visible','on')
   end   
