function SABRE2AssiModelFit(JNodevalue,Massemble,JNodevalue_i,JNodevalue_j,...
   Rval,BNodevalue,vrum_az_slider,vrum_el_slider,vrum_az_edit,vrum_el_edit,LabType,axesm,vstm)
% Developed by Woo Yong Jeong.
% Date : 07/01/2013.
% ------------------------------------------------------------------------
% -------  3D Model generator for assigning material & # of El      ------
% ------------------------------------------------------------------------
...
%             ['M',num2str(i)],'HitTest','off');
%          if isequal(strcmp(get(vstm,'Checked'),'on'),1) % white background
%             % Texting Member
%             text(sum(MJvalue(:,2))/2,-sum(MJvalue(:,4))/2-Ta,sum(MJvalue(:,3))/2,['M',num2str(i)],...
%                'VerticalAlignment','bottom','Tag','M','FontSize',9,'HitTest','on','Color','k');
%             hold on;
%          elseif isequal(strcmp(get(vstm,'Checked'),'on'),0) % black background
%             % Texting Member
%             text(sum(MJvalue(:,2))/2,-sum(MJvalue(:,4))/2-Ta,sum(MJvalue(:,3))/2,['M',num2str(i)],...
%                'VerticalAlignment','bottom','Tag','M','FontSize',9,'HitTest','on','Color','w');
%             hold on;
%          end
...
%                j4=[0;0;0];j4 = Rz*j4;                  
         ...
         text(MJvalue(1,2)+j1(1,1),MJvalue(1,4)+j1(3,1),MJvalue(1,3)+j1(2,1),'Flg1',...
            'VerticalAlignment','bottom','Tag','H','FontSize',9,'HitTest','off','Color',[0 0.9 0]);  
         text(MJvalue(1,2)+j2(1,1),MJvalue(1,4)+j2(3,1),MJvalue(1,3)+j2(2,1),'Flg2',...
            'VerticalAlignment','bottom','Tag','H','FontSize',9,'HitTest','off','Color',[0 0.9 0]);     
         end
%          text(MJvalue(2,2)+j3(1,1),MJvalue(2,4)+j3(3,1),MJvalue(2,3)+j3(2,1),'Flg1',...
%             'VerticalAlignment','bottom','Tag','H','FontSize',9,'HitTest','off','Color',[0 0.9 0]);  
%          text(MJvalue(2,2)+j4(1,1),MJvalue(2,4)+j4(3,1),MJvalue(2,3)+j4(2,1),'Flg2',...
%             'VerticalAlignment','bottom','Tag','H','FontSize',9,'HitTest','off','Color',[0 0.9 0]);             
         hold on;
         % --------------------------- Text top & bottom E 
         MJvalue = [];
...
% set(gca,'CameraViewAngle',cangle);
set(gca,'CameraPosition',cposition);
set(gca,'CameraTarget',ctarget);
