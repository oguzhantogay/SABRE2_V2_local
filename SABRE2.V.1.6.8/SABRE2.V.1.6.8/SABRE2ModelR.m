function SABRE2ModelR(JNodevalue,Massemble,JNodevalue_i,JNodevalue_j,...
...
if isempty(Massemble) % Only Joints
   ...
   for i = 1:njnode
      plot3(axesm,JNodevalue(i,2),-JNodevalue(i,4),JNodevalue(i,3),...
         'Marker','.','Color','r','MarkerSize',14,'Tag',['jn',num2str(i)],'HitTest','off','PickableParts','none');
      hold on;
   end
         ...
         for i = 1:max(BNodevalue(j,:,2))
            if isequal(BNodevalue(j,i,15),1)
               plot3(axesm,BNodevalue(j,i,3),-BNodevalue(j,i,5),...
                  ...
                  'MarkerSize',8,'Tag',['M',num2str(j),'b',num2str(i)],'HitTest','off','PickableParts','none');
               hold on;
            else
