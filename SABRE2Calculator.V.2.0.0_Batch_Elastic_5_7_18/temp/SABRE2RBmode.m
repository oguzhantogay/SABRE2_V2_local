function SABRE2RBmode(JNodevalue,Massemble,JNodevalue_i,JNodevalue_j,SNodevalue,RNCc,NCc,Nshe1,Nshe2,...
   ...
   The1,The2,SGhe1,SGhe2,Bhg,Thg,CSg,Dscale,rd_buttongroup,pt_title_name,...
...
if ~isempty(JNodevalue)
   ...
   xa=max( max(max(abs(xmax-xmin),abs(ymax-ymin)),abs(zmax-zmin))/18,(mDg+mtfb+mtft)*1.2); 
...
elseif isequal(AE,1) 
   if isequal(LIAType,0)
      set(pt_title_name,'String',['Elastic Linear Buckling Analysis Results. Applied Load Ratio = ',num2str(gammma(1,1))]) 
      set(pt_title_name,'Visible','on')     
