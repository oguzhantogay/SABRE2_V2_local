function SABRE2FlexureFun(pn_panel,pt_title_name,fim_infor_name,pd_type_subpanel,pdn_type_text,...
...
set([pdb_step_text,pdb_step_edit],'Visible','off');
set(pdstep_subpanel,'Visible','on');
% *** sub panel4
...
% *** sub panel1
set([bpm_type_text,bpm_type_name],'Visible','on')
% *** sub panel2
set([bpm_section_text,bpm_flexure_text,bpm_ni_text,bpm_nj_text],'Visible','on');
set([bpm_ni_edit,bpm_nj_edit],'Visible','on');
% *** sub panel2-1
set([bpm_fr_text,bpm_my_text,bpm_myni_text,bpm_mynj_text,bpm_myni_edit,bpm_mynj_edit],'Visible','on');
set([bpm_mz_text,bpm_mzni_text,bpm_mznj_text,bpm_mzni_edit,bpm_mznj_edit],'Visible','on');
% *** sub panel3
set([bpm_warp_text,bpm_wni_text,bpm_wnj_text],'Visible','on');
set([bpm_wni_edit,bpm_wnj_edit],'Visible','on');
% *** sub panel4
set([bpm_apply,bpm_cancel],'Visible','on'); 
...
if strcmp(get(gco,'Tag'),'EF')
   ...
   set(gco,'Color','c')    
   set(bpm_type_name,'String',num2str(sscanf(get(gco,'String'),'E%u')))
   if ~isempty(FEL)
      mxni=FEL(sscanf(get(gco,'String'),'E%u'),2);
      mxnj=FEL(sscanf(get(gco,'String'),'E%u'),3);
      myni=FEL(sscanf(get(gco,'String'),'E%u'),4);
      mynj=FEL(sscanf(get(gco,'String'),'E%u'),5);      
      mzni=FEL(sscanf(get(gco,'String'),'E%u'),6);
      mznj=FEL(sscanf(get(gco,'String'),'E%u'),7);              
      wni=FEL(sscanf(get(gco,'String'),'E%u'),8);
      wnj=FEL(sscanf(get(gco,'String'),'E%u'),9);
      
      if isequal(mxni,0) || isequal(mxnj,0) || isequal(myni,0) || isequal(mynj,0) || ...
            isequal(mzni,0) || isequal(mznj,0) || isequal(wni,0) || isequal(wnj,0)
         set(bpm_ni_edit,'Value',1);
         set(bpm_nj_edit,'Value',1);
         set(bpm_myni_edit,'Value',1);
         set(bpm_mynj_edit,'Value',1);         
         set(bpm_mzni_edit,'Value',1);
         set(bpm_mznj_edit,'Value',1);          
         set(bpm_wni_edit,'Value',1);
         set(bpm_wnj_edit,'Value',1);           
      else
         set(bpm_ni_edit,'Value',mxni);
         set(bpm_nj_edit,'Value',mxnj);
         set(bpm_myni_edit,'Value',myni);
         set(bpm_mynj_edit,'Value',mynj);         
         set(bpm_mzni_edit,'Value',mzni);
         set(bpm_mznj_edit,'Value',mznj);            
         set(bpm_wni_edit,'Value',wni);
         set(bpm_wnj_edit,'Value',wnj);  
      end
   end
end
