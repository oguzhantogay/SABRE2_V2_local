...
global The2; The2=[]; global Bhg; Bhg=[]; global Thg; Thg=[]; global CSg; CSg=[];global Mhg; Mhg=[];
...
global Myt; Myt=[]; global Phi_Mmax; Phi_Mmax=[]; global UC; UC=[]; global PrPeL; PrPeL=[];global PrPeL2nd; PrPeL2nd=[];
global LTYPE; LTYPE=[]; global cflag; cflag=0; global LNCpv; LNCpv=[];global PrPeLB; PrPeLB=[];global PrPeLNB; PrPeLNB=[];
...
%       vmsn = uimenu(vlam,'Label','Segment Number','Callback',{@view_snum_menu_Callback});
      ...
      vmpf = uimenu(vlam,'Label','Planar Fixities','Callback',{@view_pf_menu_Callback});
      ...
      vmax = uimenu(vlam,'Label','XYZ Axies','Callback',{@view_axes_menu_Callback});
...
set(vctm,'Checked','off') ;set(vdum,'Checked','on') ; set([vmjn,vmmn,vmfn,vmbf,vmpf,vmbs,vmbg,vmbw,vmbp,vmbu,vmax],'Checked','on') ; 
% ************************************************************************
% *****************                AXES              *********************
% ************************************************************************  
...
inf_title_name = uicontrol('Style','text','String','SABRE2 v1.7','Tag','ITitle',...  
...
inf_dw_name = uicontrol('Style','text','String','Donald W. White.','Tag','IText',...  
   ...
   'ForegroundColor','k','BackgroundColor',[0.9412 0.9412 0.9412],'FontSize',Fs*17,'FontWeight','bold','FontUnits','normalized');
inf_cp_name = uicontrol('Style','text','String','(Copyright 2018 All rights reserved)','Tag','IText',...  
...
lfm_member_text = uicontrol(pn_panel,'Style','text','String','',...
   ...
   'BackgroundColor','black','ForegroundColor','white','FontSize',Fs*9,'FontUnits','normalized');
% INPUT BOX using edit
lfm_height_edit = uicontrol(pn_panel,'Style','popupmenu','String',' SC| Flg2(+alpha)| Flg1(+alpha)| Cent.| Mid-Web',...
...
% ************************************************ sub panel3
lum_height_edit = uicontrol(pn_panel,'Style','popupmenu','String',' SC| Flg2| Flg1| Cent.| Mid-Web',...
   'Position',[round(Pw*(8.5/20)),round(Pd*(17.6/40)),round(Pw*(10/20)),round(Pd*(0.65/40))],...
   'FontSize',Fs*8,'FontUnits','normalized');
% Post-setting
...
ri_tau_diagram = uicontrol(pn_panel,'Style','popupmenu','enable','off','String',...
    '   None |   SRF |   Rpg |   Rpc |   Rpt |   Rh |   Myc |   Myt |   My |   J |   Phi_Mmax |   Phi_Pye |   CS UC  |   gamma*Pr/Pel ',...
...
ri_report_edit = uicontrol(pn_panel,'Style','popupmenu','enable','off','String',...
 '   None |   Axial |   Shear Y |   Shear Z |   Torque |   Moment Y |   Moment Z |   Bioment |   SRF |   Rpg |   Rpc |   Rpt |   Rh |   Myc |   Myt |   My |   J |   Phi_Mmax |   Phi_Pye |   CS UC   |   gamma*Pr/Pel ',...
...
function file_new_menu_Callback(hObject, eventdata)
   ...
   if strcmp(selection,'No')
      ...
      Qintg1=[]; Qintg2=[]; MIE=[];Sincre=[];Bhe1=[]; Bhe2=[]; The1=[]; The2=[]; Bhg=[]; Thg=[]; CSg=[]; Mhg=[];SGhe1=[]; SGhe2=[]; Mhe1=[]; Mhe2=[]; tau=[];
      LTYPE=[]; AR=0; AS=0; AE=0; AI=0; ANE=0; ANI=0; Rpg=[]; Rpc=[]; Rpt=[]; Rh=[]; Myc=[]; My=[]; Jval=[];Phi_Py=[]; UC=[]; PrPeL=[];PrPeL2nd=[];
      cflag=0; LNCpv=[];LIAType=0; NLIAType=0;HomoType=0; crLTB = 0; LGv=0; Itype=zeros(1,2);Rtype=0; AnalP=[];PrPeLB=[]; PrPeLNB=[];
      AnalP(1,1)=0;AnalP(2,1)=1;AnalP(3,1)=1;AnalP(4,1)=1;AnalP(5,1)=1;AnalP(6,1)=1; AnalP(7,1)=1;   
      set(vctm, 'Checked', 'off'); set([vmjn,vmmn,vmfn,vmbf,vmpf,vmbs,vmbg,vmbw,vmbp,vmbu,vmax],'Checked','on') ; LabType=zeros(1,10);
      cameratoolbar('Close');
      axis normal;   
...
function file_open_menu_Callback(hObject, eventdata)
   ...
   if isequal([filename,pathname],[0,0]) || isempty(filename) || isempty(pathname)
      ...
      LNC1 = []; LNC2 = []; LUEC = []; PNC = []; PNC1 = []; PNC2 = []; BNC = []; FEL = [];
      BNC1 = []; BNC2 = []; t=[]; ULoad=[]; gammma=[]; Funew =[]; FunewP =[]; FunewR =[]; QintgP=[];EGunew=[]; Qintf=[];QintfE=[];
      Qintg1=[]; Qintg2=[];MIE=[];Sincre=[]; Bhe1=[]; Bhe2=[]; The1=[]; The2=[]; Bhg=[]; Thg=[]; CSg=[]; Mhg=[];tau=[];SGhe1=[]; SGhe2=[];Mhe1=[]; Mhe2=[];
      LTYPE=[]; AR=0; AS=0; AE=0; AI=0; ANE=0; ANI=0; cflag=0; LNCpv=[];LIAType=0; crLTB = 0; LGv=0; Itype=zeros(1,2);
      NLIAType=0;HomoType=0; Rpg=[]; Rpc=[]; Rpt=[]; Rh=[]; Myc=[]; My=[]; Jval=[];Phi_Py=[]; UC=[];PrPeL=[];PrPeL2nd=[];PrPeLB=[]; PrPeLNB=[];
      ...
      [JNodevalue,Massemble,JNodevalue_i,JNodevalue_j,Rval,BNodevalue,...
         ...
         vrum_el_slider,vrum_az_edit,vrum_el_edit,LabType,axesm,vstm);
      % Drawing Nodal white points & generate NC
   if isequal(strcmp(get(vstm,'Checked'),'on'),1) % white background   
      [RNCc,Nshe1,Nshe2,DUP1,DUP2,NCc,Bhe1,Bhe2,The1,The2,SGhe1,SGhe2,Mhe1,Mhe2,Bhg,Thg,CSg,Mhg,BNC1,BNC2,FEL,error]=SABRE2LBCODEW(JNodevalue,...
   ...
   elseif isequal(strcmp(get(vstm,'Checked'),'on'),0) % black background       
      [RNCc,Nshe1,Nshe2,DUP1,DUP2,NCc,Bhe1,Bhe2,The1,The2,SGhe1,SGhe2,Mhe1,Mhe2,Bhg,Thg,CSg,Mhg,BNC1,BNC2,FEL,error]=SABRE2LBCODE(JNodevalue,...
         Massemble,JNodevalue_i,JNodevalue_j,Rval,BNodevalue,SNodevalue,...
         RNCc,Nshe1,Nshe2,DUP1,DUP2,NCc,LNC,LUEC,PNC,BNC,BNC1,BNC2,FEL,Bhe1,Bhe2,The1,The2,SGhe1,SGhe2,Mhe1,Mhe2,Bhg,Thg,CSg,Mhg,LabType,pt_title_name,axesm,vstm);    
   end
   ...
   elseif strcmp(OBJ,'res') || strcmp(OBJ,'Anal') 
      if isequal(Buc,1) 
        if ~isempty(Funew) 
            ...
            if isempty(gammma)
                if isequal(strcmp(get(vstm,'Checked'),'on'),1) % white background   
               SABRE2RBmodeW(JNodevalue,Massemble,JNodevalue_i,JNodevalue_j,SNodevalue,RNCc,NCc,Nshe1,Nshe2,DUP1,DUP2,Funew(:,:,IncreL),Rval,...
                  LNC,LUEC,PNC,PNC1,PNC2,BNC,BNC1,BNC2,FEL,Bhe1,Bhe2,The1,The2,SGhe1,SGhe2,Mhe1,Mhe2,Bhg,Thg,CSg,Mhg,Fscale,rd_buttongroup,pt_title_name,...
...
%             set(vtum, 'Checked', 'on');
         ...
         elseif isequal(Rtype,10)
            if ~isempty(Rpg)
               SABRE2DiagramSRF(JNodevalue,Massemble,JNodevalue_i,JNodevalue_j,SNodevalue,RNCc,NCc,...
                Nshe1,Nshe2,DUP1,DUP2,Rpg,Rval,pt_title_name,LabType,axesm,vstm);     
            end  
         elseif isequal(Rtype,11)
            if ~isempty(Rpc)
               SABRE2DiagramSRF(JNodevalue,Massemble,JNodevalue_i,JNodevalue_j,SNodevalue,RNCc,NCc,...
                Nshe1,Nshe2,DUP1,DUP2,Rpc,Rval,pt_title_name,LabType,axesm,vstm);     
            end  
         elseif isequal(Rtype,12)
            if ~isempty(Rpt)
               SABRE2DiagramSRF(JNodevalue,Massemble,JNodevalue_i,JNodevalue_j,SNodevalue,RNCc,NCc,...
                Nshe1,Nshe2,DUP1,DUP2,Rpt,Rval,pt_title_name,LabType,axesm,vstm);     
            end 
         elseif isequal(Rtype,13)
            if ~isempty(Rh)
               SABRE2DiagramSRF(JNodevalue,Massemble,JNodevalue_i,JNodevalue_j,SNodevalue,RNCc,NCc,...
                Nshe1,Nshe2,DUP1,DUP2,Rh,Rval,pt_title_name,LabType,axesm,vstm);     
            end  
         elseif isequal(Rtype,14)
            if ~isempty(Myc)
               Myc=round(Myc*10^1)/10^1;
               SABRE2DiagramSRF(JNodevalue,Massemble,JNodevalue_i,JNodevalue_j,SNodevalue,RNCc,NCc,...
                Nshe1,Nshe2,DUP1,DUP2,Myc,Rval,pt_title_name,LabType,axesm,vstm);     
            end 
         elseif isequal(Rtype,15)
            if ~isempty(Myt)
               Myt=round(Myt*10^1)/10^1;
               SABRE2DiagramSRF(JNodevalue,Massemble,JNodevalue_i,JNodevalue_j,SNodevalue,RNCc,NCc,...
                Nshe1,Nshe2,DUP1,DUP2,Myt,Rval,pt_title_name,LabType,axesm,vstm);     
            end 
         elseif isequal(Rtype,16)
            if ~isempty(My)
               My=round(My*10^1)/10^1;
               SABRE2DiagramSRF(JNodevalue,Massemble,JNodevalue_i,JNodevalue_j,SNodevalue,RNCc,NCc,...
                Nshe1,Nshe2,DUP1,DUP2,My,Rval,pt_title_name,LabType,axesm,vstm);     
            end 
         elseif isequal(Rtype,17)
            if ~isempty(Jval)
               SABRE2DiagramSRF(JNodevalue,Massemble,JNodevalue_i,JNodevalue_j,SNodevalue,RNCc,NCc,...
                Nshe1,Nshe2,DUP1,DUP2,Jval,Rval,pt_title_name,LabType,axesm,vstm);     
            end        
         elseif isequal(Rtype,18)
            if ~isempty(Phi_Mmax)
               Phi_Mmax=round(Phi_Mmax*10^1)/10^1;
               SABRE2DiagramSRF(JNodevalue,Massemble,JNodevalue_i,JNodevalue_j,SNodevalue,RNCc,NCc,...
                Nshe1,Nshe2,DUP1,DUP2,Phi_Mmax,Rval,pt_title_name,LabType,axesm,vstm);     
            end 
         elseif isequal(Rtype,19)
            if ~isempty(Phi_Py)
               Phi_Py=round(Phi_Py*10^1)/10^1;
               SABRE2DiagramSRF(JNodevalue,Massemble,JNodevalue_i,JNodevalue_j,SNodevalue,RNCc,NCc,...
                Nshe1,Nshe2,DUP1,DUP2,Phi_Py,Rval,pt_title_name,LabType,axesm,vstm);     
            end   
         elseif isequal(Rtype,20)
            if ~isempty(UC)
               SABRE2DiagramSRF(JNodevalue,Massemble,JNodevalue_i,JNodevalue_j,SNodevalue,RNCc,NCc,...
                Nshe1,Nshe2,DUP1,DUP2,UC,Rval,pt_title_name,LabType,axesm,vstm);     
            end 
         ...
         elseif isequal(Rtype,23)
            if ~isempty(PrPeL)
               SABRE2DiagramSRF1(JNodevalue,Massemble,JNodevalue_i,JNodevalue_j,SNodevalue,RNCc,NCc,...
                Nshe1,Nshe2,DUP1,DUP2,PrPeL,Rval,pt_title_name,LabType,axesm,vstm);     
            end
         elseif isequal(Rtype,24)   
            IncreL = get(ri_cancel,'Value');
            Itype(1,1)=7;Itype(1,2)=IncreL;          
            if ~isempty(PrPeL2nd)
              SABRE2DiagramSRF1(JNodevalue,Massemble,JNodevalue_i,JNodevalue_j,SNodevalue,RNCc,NCc,...
                Nshe1,Nshe2,DUP1,DUP2,PrPeL2nd(:,:,IncreL),Rval,pt_title_name,LabType,axesm,vstm);     
            end
         elseif isequal(Rtype,25)   
            IncreL = get(ri_cancel,'Value');
            Itype(1,1)=7;Itype(1,2)=IncreL;          
            if ~isempty(PrPeLB)
              SABRE2DiagramSRF1(JNodevalue,Massemble,JNodevalue_i,JNodevalue_j,SNodevalue,RNCc,NCc,...
                Nshe1,Nshe2,DUP1,DUP2,PrPeLB(:,:,IncreL),Rval,pt_title_name,LabType,axesm,vstm);     
            end   
         elseif isequal(Rtype,26)
            if ~isempty(PrPeLNB)
               SABRE2DiagramSRF1(JNodevalue,Massemble,JNodevalue_i,JNodevalue_j,SNodevalue,RNCc,NCc,...
                Nshe1,Nshe2,DUP1,DUP2,PrPeLNB,Rval,pt_title_name,LabType,axesm,vstm);     
            end            
         else
            SABRE2ModelFit(JNodevalue,Massemble,JNodevalue_i,JNodevalue_j, ...
               Rval,BNodevalue,vrum_az_slider,vrum_el_slider,vrum_az_edit,vrum_el_edit,LabType,axesm,vstm);   
...
function view_axes_menu_Callback(hObject, eventdata)
   clc; 
   if strcmp(get(gcbo, 'Checked'),'on')     
      set(vmax, 'Checked', 'off');LabType(1,3)=1;
   else
      set(vmax, 'Checked', 'on');LabType(1,3)=0;
   end   
   if isempty(OBJ)
       if isempty(RNCc)
          OBJ='define';
...
function view_bf_menu_Callback(hObject, eventdata)
   clc;
   if strcmp(get(gcbo, 'Checked'),'on')     
      set([vmbf,vmpf], 'Checked', 'off');LabType(1,5)=1;       
   else
      set([vmbf,vmpf], 'Checked', 'on');LabType(1,5)=0;      
   end
   if isequal(strcmp(get(vstm,'Checked'),'on'),1) % white background   
      [RNCc,Nshe1,Nshe2,DUP1,DUP2,NCc,Bhe1,Bhe2,The1,The2,SGhe1,SGhe2,Mhe1,Mhe2,Bhg,Thg,CSg,Mhg,BNC1,BNC2,FEL,error]=SABRE2LBCODEW(JNodevalue,...
         Massemble,JNodevalue_i,JNodevalue_j,Rval,BNodevalue,SNodevalue,...
...
function view_pf_menu_Callback(hObject, eventdata)
   clc;
   if strcmp(get(gcbo, 'Checked'),'on')     
      set([vmbf,vmpf], 'Checked', 'off');LabType(1,5)=1;       
   else
      set(vmpf, 'Checked', 'on');LabType(1,5)=2;      
   end
   if isequal(strcmp(get(vstm,'Checked'),'on'),1) % white background   
      [RNCc,Nshe1,Nshe2,DUP1,DUP2,NCc,Bhe1,Bhe2,The1,The2,SGhe1,SGhe2,Mhe1,Mhe2,Bhg,Thg,CSg,Mhg,BNC1,BNC2,FEL,error]=SABRE2LBCODEW(JNodevalue,...
         Massemble,JNodevalue_i,JNodevalue_j,Rval,BNodevalue,SNodevalue,...
...
function view_screenshot_menu_Callback(hObject, eventdata)
   ...
   if strcmp(get(gcbo, 'Checked'),'on')     
      ...
      set(findobj('Tag','NsheL'),'Color','y');set(findobj('Tag','DAi'),'EdgeColor','c');set(findobj('Tag','DBi'),'EdgeColor','c');set(findobj('Tag','DMt'),'EdgeColor','c');
      set(findobj('Tag','DMy'),'EdgeColor','c');set(findobj('Tag','DMz'),'EdgeColor','c');set(findobj('Tag','DSy'),'EdgeColor','c');set(findobj('Tag','DSz'),'EdgeColor','c');
      if ~isempty(RNCc)
         ...
         for i = 1:length(DUP1(:,1))
             set(findobj('Tag',['Ai',num2str(i)]),'Color','y');set(findobj('Tag',['Sy',num2str(i)]),'Color','y');
             set(findobj('Tag',['Sz',num2str(i)]),'Color','y');set(findobj('Tag',['Mt',num2str(i)]),'Color','y');
             set(findobj('Tag',['My',num2str(i)]),'Color','y');set(findobj('Tag',['Mz',num2str(i)]),'Color','y');
             set(findobj('Tag',['Bi',num2str(i)]),'Color','y');set(findobj('Tag',['SRF',num2str(i)]),'Color','y');
         end
      end
   else
      set(vstm, 'Checked', 'on');
      ...
      set(findobj('Tag','NsheL'),'Color',[0.4 0.4 0.4]);set(findobj('Tag','DAi'),'EdgeColor',[0 0 0.8]);set(findobj('Tag','DBi'),'EdgeColor',[0 0 0.8]);set(findobj('Tag','DMt'),'EdgeColor',[0 0 0.8]);
      set(findobj('Tag','DMy'),'EdgeColor',[0 0 0.8]);set(findobj('Tag','DMz'),'EdgeColor',[0 0 0.8]);set(findobj('Tag','DSy'),'EdgeColor',[0 0 0.8]);set(findobj('Tag','DSz'),'EdgeColor',[0 0 0.8]);
      if ~isempty(RNCc)
         for i = 1:length(RNCc(:,1))
         ...
         for i = 1:length(DUP1(:,1))
             set(findobj('Tag',['Ai',num2str(i)]),'Color',[0.4 0.4 0.4]);set(findobj('Tag',['Sy',num2str(i)]),'Color',[0.4 0.4 0.4]);
             set(findobj('Tag',['Sz',num2str(i)]),'Color',[0.4 0.4 0.4]);set(findobj('Tag',['Mt',num2str(i)]),'Color',[0.4 0.4 0.4]);
             set(findobj('Tag',['My',num2str(i)]),'Color',[0.4 0.4 0.4]);set(findobj('Tag',['Mz',num2str(i)]),'Color',[0.4 0.4 0.4]);
             set(findobj('Tag',['Bi',num2str(i)]),'Color',[0.4 0.4 0.4]);set(findobj('Tag',['SRF',num2str(i)]),'Color',[0.4 0.4 0.4]);
         end         
      end      
   end 
...
function property_assign_menu_Callback(hObject,eventdata) 
   ...
   if isempty(SNodevalue)
      RNCc = []; NCc = []; Nshe1 = []; Nshe2 = []; DUP1 = []; DUP2 = []; LNC = [];
      LNC1 = []; LNC2 = []; LUEC = []; PNC = []; PNC1 = []; PNC2 = []; BNC = []; FEL = [];
      BNC1 = []; BNC2 = []; t=[]; ULoad=[]; gammma=[]; Funew =[]; FunewP =[];FunewR =[];QintgP=[];EGunew=[]; Qintf=[];QintfE=[];PrPeL2nd=[];PrPeL=[];PrPeLB=[];PrPeLNB=[];
      Qintg1=[]; Qintg2=[]; MIE =[]; Sincre=[]; Bhe1=[]; Bhe2=[]; The1=[]; The2=[]; Bhg=[]; Thg=[]; CSg=[]; Mhg=[];tau=[];SGhe1=[]; SGhe2=[]; Mhe1=[]; Mhe2=[];    
   end  
...
function analysis_2ndelastic_menu_Callback(hObject, eventdata)
   ...
   if isempty(RNCc) || isempty(PNC)     
      ...
      if isequal(max(max(abs(LNC(:,5:10)))),0) && isequal(max(max(abs(LUEC(:,5:7)))),0) && isequal(strcmp(get(get(ap_sw_buttongroup,'SelectedObject'),'Tag'),'sw_on'),0)
         ...
         lambda = str2double(get(api_assign_edit,'String'));   
         ninc = str2double(get(apninc_assign_edit,'String'));           
         [AS,Funew,Qintf,QintfE,QintgP,Qintg1,Qintg2,MIE,Sincre,PrPeL2nd] = Analysis2nd(Massemble,Rval,...
   ...
   if isempty(RNCc) || isempty(PNC)     
      ...
      if isequal(max(max(abs(LNC(:,5:10)))),0) && isequal(max(max(abs(LUEC(:,5:7)))),0) && isequal(strcmp(get(get(ap_sw_buttongroup,'SelectedObject'),'Tag'),'sw_on'),0)
         ...
         switch get(get(ap_jval_buttongroup,'SelectedObject'),'Tag')
            case 'jval_on' 
               [AE,gammma,FunewR,EGunew,Qintf,QintfE,Qintg1,Qintg2,MIE,PrPeLB] = AlanysisBucklingESL(Massemble,...
            ...
            case 'jval_off'
               [AE,gammma,FunewR,EGunew,Qintf,QintfE,Qintg1,Qintg2,MIE,PrPeLB] = AlanysisBuckling(Massemble,...
                  Rval,JNodevalue_i,JNodevalue_j,SNodevalue,RNCc,Nshe1,Nshe2,DUP1,DUP2,LNC,LNC1,LNC2,...
...
function analysis_icba_Design_menu_Callback(hObject, eventdata)
   ...
   if isempty(RNCc) || isempty(PNC)     
      ...
      if isequal(max(max(abs(LNC(:,5:10)))),0) && isequal(max(max(abs(LUEC(:,5:7)))),0) && isequal(strcmp(get(get(ap_sw_buttongroup,'SelectedObject'),'Tag'),'sw_on'),0)
         ...
         [AI,gammma,FunewR,EGunew,Qintf,QintfE,Qintg1,Qintg2,MIE,tau,Rpg,Rpc,Rpt,Rh,Myc,Myt,My,Jval,Phi_Mmax,Phi_Py,UC,PrPeL,crLTB,LGv] = AlanysisIEBuckling(Massemble,...
            Rval,JNodevalue_i,JNodevalue_j,SNodevalue,RNCc,Nshe1,Nshe2,DUP1,DUP2,LNC,LNC1,LNC2,...
...
function analysis_icba_Check_menu_Callback(hObject, eventdata)
   ...
   if isempty(RNCc) || isempty(PNC)     
      ...
      if isequal(max(max(abs(LNC(:,5:10)))),0) && isequal(max(max(abs(LUEC(:,5:7)))),0) && isequal(strcmp(get(get(ap_sw_buttongroup,'SelectedObject'),'Tag'),'sw_on'),0)
         ...
         APP=0;AR=0;AS=0;AE=0;AI=0;ANE=0;ANI=0;lambda = 1;nmode = 1;ninc = 1;FunewP=[];QintgP=[];
         JeongP=str2double(get(ap_jeong_edit,'String'));BrentP=str2double(get(ap_brent_edit,'String'));  
         [AI,gammma,FunewR,EGunew,Qintf,QintfE,Qintg1,Qintg2,MIE,tau,Rpg,Rpc,Rpt,Rh,Myc,Myt,My,Jval,Phi_Mmax,Phi_Py,UC,PrPeL,crLTB,LGv] = AlanysisIEBucklingCheck(Massemble,...
            Rval,JNodevalue_i,JNodevalue_j,SNodevalue,RNCc,Nshe1,Nshe2,DUP1,DUP2,LNC,LNC1,LNC2,...
            LUEC,PNC,PNC1,PNC2,BNC,BNC1,BNC2,FEL,nmode,ninc,lambda,pt_title_name,...
            ap_sw_buttongroup,ap_cv_buttongroup,JeongP,BrentP,LIAType); 
         set(ri_scale_edit,'String',num2str(1.2*max(max(RNCc(:,5)),max(RNCc(:,7)))));
...
function analysis_nonecba_menu_Callback(hObject, eventdata)
   ...
   if isempty(RNCc) || isempty(PNC)     
      ...
      if isequal(max(max(abs(LNC(:,5:10)))),0) && isequal(max(max(abs(LUEC(:,5:7)))),0) && isequal(strcmp(get(get(ap_sw_buttongroup,'SelectedObject'),'Tag'),'sw_on'),0)
         ...
         switch get(get(ap_jval_buttongroup,'SelectedObject'),'Tag')
            case 'jval_on'       
               [ANE,gammma,FunewR,EGunew,Qintf,QintfE,Qintg1,Qintg2,MIE,PrPeLNB] = AnalysisNonBucklingESL(Massemble,...
                  Rval,JNodevalue_i,JNodevalue_j,SNodevalue,RNCc,Nshe1,Nshe2,DUP1,DUP2,LNC,LNC1,LNC2,...
                  LUEC,PNC,PNC1,PNC2,BNC,BNC1,BNC2,FEL,nmode,ninc,lambda,pt_title_name,...
                  ap_sw_buttongroup,ap_da_buttongroup,ap_cv_buttongroup,JeongP,BrentP,NLIAType);    

            case 'jval_off'
               [ANE,gammma,FunewR,EGunew,Qintf,QintfE,Qintg1,Qintg2,MIE,PrPeLNB] = AnalysisNonBuckling(Massemble,...
                  Rval,JNodevalue_i,JNodevalue_j,SNodevalue,RNCc,Nshe1,Nshe2,DUP1,DUP2,LNC,LNC1,LNC2,...
                  LUEC,PNC,PNC1,PNC2,BNC,BNC1,BNC2,FEL,nmode,ninc,lambda,pt_title_name,...
...
function analysis_nonicba_Design_menu_Callback(hObject, eventdata)
   ...
   if isempty(RNCc) || isempty(PNC)     
      ...
      if isequal(max(max(abs(LNC(:,5:10)))),0) && isequal(max(max(abs(LUEC(:,5:7)))),0) && isequal(strcmp(get(get(ap_sw_buttongroup,'SelectedObject'),'Tag'),'sw_on'),0)
         ...
         APP=0;AR=0;AS=0;AE=0;AI=0;ANE=0;ANI=0;lambda = 1;nmode = 1;ninc = 1;FunewP=[];QintgP=[];
         JeongP=str2double(get(ap_jeong_edit,'String'));BrentP=str2double(get(ap_brent_edit,'String'));    
         [ANI,gammma,FunewR,EGunew,Qintf,QintfE,Qintg1,Qintg2,MIE,tau,Rpg,Rpc,Rpt,Rh,Myc,Myt,My,Jval,Phi_Mmax,Phi_Py,UC,PrPeL,crLTB,LGv] = AnalysisNonIEBuckling(Massemble,...
            Rval,JNodevalue_i,JNodevalue_j,SNodevalue,RNCc,Nshe1,Nshe2,DUP1,DUP2,LNC,LNC1,LNC2,...
            LUEC,PNC,PNC1,PNC2,BNC,BNC1,BNC2,FEL,nmode,ninc,lambda,pt_title_name,...
            ap_sw_buttongroup,ap_da_buttongroup,ap_cv_buttongroup,JeongP,BrentP,NLIAType);   
         set(ri_scale_edit,'String',num2str(1.2*max(max(RNCc(:,5)),max(RNCc(:,7)))));
...
function analysis_nonicba_Check_menu_Callback(hObject, eventdata)
   ...
   if isempty(RNCc) || isempty(PNC)     
      ...
      if isequal(max(max(abs(LNC(:,5:10)))),0) && isequal(max(max(abs(LUEC(:,5:7)))),0) && isequal(strcmp(get(get(ap_sw_buttongroup,'SelectedObject'),'Tag'),'sw_on'),0)
         ...
         APP=0;AR=0;AS=0;AE=0;AI=0;ANE=0;ANI=0;lambda = 1;nmode = 1;ninc = 1;FunewP=[];QintgP=[];
         JeongP=str2double(get(ap_jeong_edit,'String'));BrentP=str2double(get(ap_brent_edit,'String'));    
         [ANI,gammma,FunewR,EGunew,Qintf,QintfE,Qintg1,Qintg2,MIE,tau,Rpg,Rpc,Rpt,Rh,Myc,Myt,My,Jval,Phi_Mmax,Phi_Py,UC,PrPeL,crLTB,LGv] = AnalysisNonIEBucklingCheck(Massemble,...
            Rval,JNodevalue_i,JNodevalue_j,SNodevalue,RNCc,Nshe1,Nshe2,DUP1,DUP2,LNC,LNC1,LNC2,...
            LUEC,PNC,PNC1,PNC2,BNC,BNC1,BNC2,FEL,nmode,ninc,lambda,pt_title_name,...
            ap_sw_buttongroup,ap_da_buttongroup,ap_cv_buttongroup,JeongP,BrentP,NLIAType);   
         set(ri_scale_edit,'String',num2str(1.2*max(max(RNCc(:,5)),max(RNCc(:,7)))));
...
function pdn_apply_pushbutton_Callback(hObject, eventdata)
   clc; pan off; zoom off; rotate3d off; set([vzm,vrm,vpm], 'Checked', 'off');
   dunit=get(punit_edit,'Value');set(vmjn, 'Checked', 'on');LabType(1,1)=0; 
   if isequal(dunit,2)
      AnalP(7,1)=2;
   else
      AnalP(7,1)=1;
   ...
   [JNodevalue,JNodevalue_i,JNodevalue_j,BNodevalue,SNodevalue]=SABRE2JointApp(JNodevalue,...
      ...
      pdn_coordy_edit,pdn_coordz_edit,pt_title_name,LabType,axesm,vstm); 
end % function end
% --------------------------------------------------------------------
...
function pde_set_pushbutton_Callback(hObject, eventdata)
   ...
   elseif wnum > 1
      ...
      if isequal(dunit,2)
         ...
         set(pde_twj_edit,'String',num2str(SECTION((wnum-1),6)*2.54));
...
function pde_apply_pushbutton_Callback(hObject, eventdata)
   clc; pan off; zoom off; rotate3d off; set([vzm,vrm,vpm], 'Checked', 'off');
   set(vmmn, 'Checked', 'on');LabType(1,2)=0; set(vmfn, 'Checked', 'on');LabType(1,4)=0; 
...
function pdmi_apply_pushbutton_Callback(hObject, eventdata)  
   ...
   if strcmp(selection,'No')
      ...
      Qintg1=[]; Qintg2=[]; MIE =[]; Sincre=[]; Bhe1=[]; Bhe2=[]; The1=[]; The2=[]; Bhg=[]; Thg=[]; CSg=[]; Mhg=[];tau=[];SGhe1=[]; SGhe2=[];Mhe1=[]; Mhe2=[];   
      set([vmjn,vmbf,vmpf,vmbs,vmbg,vmbw,vmbp,vmbu,vmax],'Checked','on') ; LabType=zeros(1,10);
      ...
      % Drawing Nodal white points & generate NC
   if isequal(strcmp(get(vstm,'Checked'),'on'),1) % white background   
      [RNCc,Nshe1,Nshe2,DUP1,DUP2,NCc,Bhe1,Bhe2,The1,The2,SGhe1,SGhe2,Mhe1,Mhe2,Bhg,Thg,CSg,Mhg,BNC1,BNC2,FEL,error,LNC,LNC1,LNC2,LUEC,PNC,PNC1,PNC2,BNC]=SABRE2MLCODEW(JNodevalue,...
         Massemble,JNodevalue_i,JNodevalue_j,Rval,BNodevalue,SNodevalue,...
         RNCc,Nshe1,Nshe2,DUP1,DUP2,NCc,LNC,LNC1,LNC2,LUEC,PNC,PNC1,PNC2,BNC,BNC1,BNC2,FEL,Bhe1,Bhe2,The1,The2,SGhe1,SGhe2,Mhe1,Mhe2,Bhg,Thg,CSg,Mhg,pdmi_name,pdmi_type_edit,LabType,pt_title_name,axesm,vstm);    
   elseif isequal(strcmp(get(vstm,'Checked'),'on'),0) % black background       
      [RNCc,Nshe1,Nshe2,DUP1,DUP2,NCc,Bhe1,Bhe2,The1,The2,SGhe1,SGhe2,Mhe1,Mhe2,Bhg,Thg,CSg,Mhg,BNC1,BNC2,FEL,error,LNC,LNC1,LNC2,LUEC,PNC,PNC1,PNC2,BNC]=SABRE2MLCODE(JNodevalue,...
         Massemble,JNodevalue_i,JNodevalue_j,Rval,BNodevalue,SNodevalue,...
         RNCc,Nshe1,Nshe2,DUP1,DUP2,NCc,LNC,LNC1,LNC2,LUEC,PNC,PNC1,PNC2,BNC,BNC1,BNC2,FEL,Bhe1,Bhe2,The1,The2,SGhe1,SGhe2,Mhe1,Mhe2,Bhg,Thg,CSg,Mhg,pdmi_name,pdmi_type_edit,LabType,pt_title_name,axesm,vstm); 
   end
      end
...
function lfm_apply_pushbutton_Callback(hObject, eventdata)
   ...
   if isequal(strcmp(get(vstm,'Checked'),'on'),1) % white background   
   [LNC,LNC1,LNC2]=SABRE2ForcAppW(JNodevalue,Massemble,Rval,...
...
function lfm_cancel_pushbutton_Callback(hObject, eventdata)
   ...
   if isequal(strcmp(get(vstm,'Checked'),'on'),1) % white background   
      [LNC,LNC1,LNC2]=SABRE2ForcResW(JNodevalue,Massemble,Rval,...
...
function lum_cancel_pushbutton_Callback(hObject, eventdata)
   ...
   if isequal(strcmp(get(vstm,'Checked'),'on'),1) % white background   
      [LUEC]=SABRE2UnifResW(JNodevalue,Massemble,Rval,...
         RNCc,DUP1,DUP2,NCc,LNC,LUEC,ULoad,SNodevalue,Nshe1,Nshe2,Bhe1,Bhe2,The1,The2,SGhe1,SGhe2,Mhe1,Mhe2,Bhg,Thg,CSg,Mhg,lum_type_mem,lum_type_seg,...
         lum_height_edit,lum_wx_edit,lum_wy_edit,lum_wz_edit,t,pt_title_name,LabType,axesm,vstm);    
   elseif isequal(strcmp(get(vstm,'Checked'),'on'),0) % black background       
...
function bfm_cancel_pushbutton_Callback(hObject, eventdata)
   ...
   if isequal(strcmp(get(vstm,'Checked'),'on'),1) % white background   
      [PNC,PNC1,PNC2]=SABRE2BConResW(JNodevalue,JNodevalue_i,JNodevalue_j,...
         Massemble,Rval,RNCc,DUP1,DUP2,PNC,PNC1,PNC2,Bhg,Thg,CSg,bfm_type_name,...
         bfm_coordx_edit,bfm_coordy_edit,bfm_coordz_edit,bfm_ux_edit,...
         bfm_uy_edit,bfm_uz_edit,bfm_rx_edit,bfm_ry_edit,bfm_rz_edit,...
         bfm_phix_edit,bfm_height_edit,bfm_ux_buttongroup,...
...
function ria_diagram_pushbutton_Callback(hObject, eventdata)
   ...
   if ~isempty(Qintf)
      ...
      SABRE2DiagramAi(JNodevalue,Massemble,JNodevalue_i,JNodevalue_j,...
         ...
         pt_title_name,QintfE(:,:,IncreL),LabType,axesm,vstm);
      if strcmp(get(vtum, 'Checked'),'on')  
         set(findobj('Type','text'),'Visible','on'); set(findobj('Tag','axis'),'Visible','on');
         set(findobj('Tag','EF'),'Visible','off');set(findobj('Tag','S'),'Visible','off');
      else
   ...
   if ~isempty(Qintf)  
      if isempty(gammma)
         SABRE2DiagramReport(JNodevalue,Massemble,SNodevalue,RNCc,NCc,...
            Nshe1,Nshe2,DUP1,DUP2,Funew(:,:,IncreL),Qintf(:,:,IncreL),tau,Rpg,Rpc,Rpt,Rh,Myc,Myt,My,Jval,Phi_Mmax,Phi_Py,UC,PrPeL,PrPeL2nd(:,:,IncreL),PrPeLB(:,:,IncreL),PrPeLNB,Rval,...
