function [Kt,Kgg,Qino,Kh2,Qin_into,Pt,Mzt]=StiffnessELEMENT3DKGDM(bfb,tfb,bft,tft,Dg,yc,ht,hb,Dt,Db,hst,... 
    hsb,ys,tw,Afillet,L,E,G,fillet,hstp,hsbp,ysp,bftp,bfbp,...
    theta1x,theta1z,theta1y,theta2x,theta2z,theta2y,theta1xp,theta2xp,qno,qy,dhn,Pbar,FEL,slender,ap_da_buttongroup,angletap,CSangel,et1,et2,et3,taub)
% Developed by Woo Yong Jeong.
% Date : 12/01/2012.
% ************************************************************************
% ***************        Updated Kg by updated Fn         ****************
% ************************************************************************
...
elseif isequal(dhn,4) % Centroid
   Kh2=qy*(yuc(1))*wts(1)*H(:,:,1)+...
      ...
      qy*(yuc(4))*wts(4)*H(:,:,4)+...
      qy*(yuc(5))*wts(5)*H(:,:,5);   
elseif isequal(dhn,5) % Mid-Web
    Kh2=qy*(hste(1)-tfte(1)/2 - De(1)/2)*wts(1)*H(:,:,1)+...
      qy*(hste(2)-tfte(2)/2 - De(2)/2)*wts(2)*H(:,:,2)+...
      qy*(hste(3)-tfte(3)/2 - De(3)/2)*wts(3)*H(:,:,3)+...
      qy*(hste(4)-tfte(4)/2 - De(4)/2)*wts(4)*H(:,:,4)+...
      qy*(hste(5)-tfte(5)/2 - De(5)/2)*wts(5)*H(:,:,5);   
end
