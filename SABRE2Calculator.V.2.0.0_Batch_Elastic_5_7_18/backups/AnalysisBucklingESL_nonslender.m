function [AE,gammma,FunewR,EGunew,Qintf,QintfE,Qint1,Qint2,Mz,yc,Ixx,xg1,xg2,rt,Shear_center,Axial] = AnalysisBucklingESL_nonslender(Massemble,...
...
PrPeLB=zeros(5,xn,(round(ninc/lambda))); % Preallocationg
...
for i=1:xn
   if isequal(LNC1(i,5),0) && isequal(LNC1(i,6),0)
      ...
      elseif isequal(LNC1(i,13),3) % Mid-web
         Lrd1(i,1)=-(-hsb1(i,1)+hb1(i,1));
...
if ~isempty(LNC)
   ...
   for i=1:xn
      ...
      elseif isequal(LNC1(i,13),3) % Mid-Web
         hd1(i,1)=-hsb1(i,1)+hb1(i,1)+LNC1(i,12); 
      elseif isequal(LNC1(i,13),4) % Centroid
...
%       Rsd(3,4)=rd1(n);  Rsd(10,11)=rd2(n); 
   ...
   for n = 1:xn
      ...
      [slender(:,n),Mz_temp,yc_temp,Ix_temp,rt_temp,Shear_center_temp]=StiffnessELEMENT3DESL_nonslender([bfb1(n);bfb2(n)],...
         [tfb1(n);tfb2(n)],[bft1(n);bft2(n)],[tft1(n);tft2(n)],...
         [Dg1(n);Dg2(n)],[yc1(n);yc2(n)],[ht1(n);ht2(n)],[hb1(n);hb2(n)], ...
         [Dt1(n);Dt2(n)],[Db1(n);Db2(n)],[hst1(n);hst2(n)],[hsb1(n);hsb2(n)], ...
         [ys1(n);ys2(n)],[tw1(n);tw2(n)],[Afillet1(n),Afillet2(n)],L0(n,1),E(n,1),G(n,1),fillet(n,:),...
         hstp(n),hsbp(n),ysp(n),bftp(n),bfbp(n), ...
         theta1xn,theta1zn,theta1yn,theta2xn,theta2zn,theta2yn,...
         theta1xpn,theta2xpn,qno,Fy(n,1),Fyfi(n,1),Fyw(n,1),Fyfo(n,1),[ytbar1(n);ytbar2(n)],Pbar,FEL(n,:),LIAType,HomoType(n,1),angletap(n,2));


      Mz(n,:)=Mz_temp;yc(n,:)=yc_temp;Ixx(n,:)=Ix_temp;rt(n,:)=rt_temp;Shear_center(n,:)=Shear_center_temp;
...
%       Rsd(3,4)=rd1(n);  Rsd(10,11)=rd2(n); 
   ...
   for i = 1:xn
      ...
      for i = 1:xn
          for j = 1:5
                  PrPeLB(j,i,increment) = PrPeL(j,i);
          end
      end      
   % ************************************** Wait2
   perc = increment;
...
if isequal(Bflag,1)
   delete(wh)
   set(pt_title_name,'String','Elastic Linear Buckling Analysis did not finish. Please try to check loads and boundry conditions.')
...
elseif isequal(Bflag,2)
   delete(wh)
   set(pt_title_name,'String','Elastic Linear Buckling Analysis did not finish. Please try to use the other Algorithm or Scaled Parameter.')
   set(pt_title_name,'Visible','on')
   gammma=0;   
   Qintf=Qintf*0;
   QintfE=QintfE*0;
...
%    end
   ...
   for i = 1:nmode
      for j=1:length(Qintf(:,1))
         for k=1:14
            Qintf(j,k,i)=Qintf(j,k)*gammma(i,1);
         end
      end
   end
   ...
   for i = 1:nmode
      for j=1:length(Qintf(:,1))
         for k=1:14
            QintfE(j,k,i)=QintfE(j,k)*gammma(i,1);
         end
      end
   end
   ...
   for i = 1:nmode
      for j=1:length(Qintf(:,1))
         for k=1:5
            PrPeLB(k,j,i)=PrPeL(k,j)*gammma(i,1);
         end
      end
   end  
   
   for i = 1:nmode
      for j=1:length(Qint1(:,1))
         for k=1:7
            Qint1(j,k,i)=Qint1(j,k)*gammma(i,1);
         end
      end
   end

   for i = 1:nmode
      for j=1:length(Qint2(:,1))
         for k=1:7
            Qint2(j,k,i)=Qint2(j,k)*gammma(i,1);
         end
      end
   end
   ...
   set(pt_title_name,'String',['Elastic Linear Buckling Analysis is completed. Applied Load Ratio = ',num2str(gammma(1,1))])
   set(pt_title_name,'Visible','on')
...
AE=1; % Set linear elastic analysis
