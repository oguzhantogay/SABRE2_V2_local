function taub=StiffnessELEMENT3Dtaub(bfb,tfb,bft,tft,Dg,yc,ht,hb,Dt,Db,hst,... 
   hsb,ys,tw,Afillet,L,E,G,fillet,hstp,hsbp,ysp,bftp,bfbp,...
   theta1x,theta1z,theta1y,theta2x,theta2z,theta2y,theta1xp,theta2xp,qno,Fy,Fyfi,Fyw,Fyfo,ytbar,Pbar,FEL,HomoType,angletap,CSangel)
...
for i=1:nip
   % Compression flange
   if bfcom(1,i)/(2*tfcom(1,i)) <= 0.64*sqrt(kc(1,i)*E/Fyf)*sqrt(Fyf/Fcr)
      bfcomf(1,i)=bfcom(1,i);
   else
      Few(1,i)= (cf2*0.64*sqrt(kc(1,i)*E/Fyf)/(bfcom(1,i)/(2*tfcom(1,i))))^2*Fyf;
      bfcomf(1,i)=(1-cf1*sqrt(Few(1,i)/Fcr))*sqrt(Few(1,i)/Fcr)*bfcom(1,i);
   end
   % Tension flange
   if bften(1,i)/(2*tften(1,i)) <= 0.64*sqrt(kc(1,i)*E/Fyf)*sqrt(Fyf/Fcr)
      bftenf(1,i)=bften(1,i);
   else
      Few(1,i)= (cf2*0.64*sqrt(kc(1,i)*E/Fyf)/(bften(1,i)/(2*tften(1,i))))^2*Fyf;
      bftenf(1,i)=(1-cf1*sqrt(Few(1,i)/Fcr))*sqrt(Few(1,i)/Fcr)*bften(1,i);
   end  
   % web
   if De_fillet(1,i)/twe(1,i) <= 1.49*sqrt(E/Fyfw)*sqrt(Fyfw/Fcr)
      Def(1,i)=De_fillet(1,i);
   else
      Few(1,i)= (cw2*1.49*sqrt(E/Fyfw)/(De_fillet(1,i)/twe(1,i)))^2*Fyfw;
      Def(1,i)=(1-cw1*sqrt(Few(1,i)/Fcr))*sqrt(Few(1,i)/Fcr)*De_fillet(1,i);
   end
...
if isequal(HomoType,0)
   for i=1:nip
      Pye(1,i)=Fycom*Aef(1,i);
   end
else
   for i=1:nip
      Pye(1,i)=( bfcomf(1,i)*tfcom(1,i)*Fyf + bftenf(1,i)*tften(1,i)*Fyf ...
...
taub=zeros(1,nip);
for i=1:nip
   if abs(Pt(1,i))/(Pye(1,i)) > 0.5
      taub(1,i) = 4*(abs(Pt(1,i))/(Pye(1,i)))*(1 - abs(Pt(1,i))/(Pye(1,i)));
   else
      taub(1,i) = 1; 
   end
end
