function [LTYPE]=SABRE2ForcFun(pn_panel,pt_title_name,fim_infor_name,pd_type_subpanel,pdn_type_text,...
...
% *** sub panel1
set([lfm_type_text,lfm_type_name],'Visible','on')
% *** sub panel2
set([lfm_coordx_edit,lfm_coordy_edit,lfm_coordz_edit],'Visible','on');
set([lfm_ld_text,lfm_ld_edit,lfm_all_apply,lfm_clc_apply],'Visible','on')
% *** sub panel3
set([lfm_height_text,lfm_member_text],'Visible','on');
set([lfm_height_edit,lfm_alpha_text,lfm_alpha_edit],'Visible','on');
% *** sub panel4
set(lfm_fc_text,'Visible','on');
set([lfm_fx_text,lfm_fy_text,lfm_fz_text],'Visible','on');
set([lfm_mx_text,lfm_my_text,lfm_mz_text],'Visible','on');
set([lfm_fx_edit,lfm_fy_edit,lfm_fz_edit,lfm_mx_edit,...
   lfm_my_edit,lfm_mz_edit],'Visible','on');
% *** sub panel5
set([lfm_apply,lfm_cancel],'Visible','on'); 
...
% -----------------------        EDIT INUPT       ------------------------
% ------------------------------------------------------------------------ 
if isempty(get(lfm_ld_edit,'String'))
   LTYPE=[];
end
...
if ~isempty(get(gco,'Tag'))
   if isempty(LNC)
      ...
      % shear center
      if isempty(LTYPE)
         LTYPE(1,1)=RNCc(sscanf(get(gco,'Tag'),'RNCc%u'),1);
      else
         LTYPE(1,1+length(LTYPE(1,:)))=RNCc(sscanf(get(gco,'Tag'),'RNCc%u'),1);
      end
      LTYPE=unique(LTYPE);
      set(lfm_ld_edit,'String',num2str(LTYPE))
      set(lfm_type_name,'String',num2str(RNCc(sscanf(get(gco,'Tag'),'RNCc%u'),1)))
      set(lfm_coordx_edit,'String',num2str(RNCc(sscanf(get(gco,'Tag'),'RNCc%u'),2)))
      set(lfm_coordy_edit,'String',num2str(RNCc(sscanf(get(gco,'Tag'),'RNCc%u'),3)))
      set(lfm_coordz_edit,'String',num2str(RNCc(sscanf(get(gco,'Tag'),'RNCc%u'),4))) 
      set(lfm_fx_edit,'String',num2str(0))
      set(lfm_fy_edit,'String',num2str(0))
      set(lfm_fz_edit,'String',num2str(0))
      set(lfm_mx_edit,'String',num2str(0))
      set(lfm_my_edit,'String',num2str(0))
      set(lfm_mz_edit,'String',num2str(0)) 
   else
      if isequal(strcmp(get(vstm,'Checked'),'on'),1) % white background
         set(findobj('Color','c'),'Color',[0 0.5 1])
      elseif isequal(strcmp(get(vstm,'Checked'),'on'),0) % black background
      ...
      if strcmp(get(gco,'Type'),'line')
         ...
         set(lfm_coordy_edit,'String',num2str(RNCc(sscanf(get(gco,'Tag'),'RNCc%u'),3)))
         set(lfm_coordz_edit,'String',num2str(RNCc(sscanf(get(gco,'Tag'),'RNCc%u'),4))) 
         set(lfm_fx_edit,'String',num2str(LNC(sscanf(get(gco,'Tag'),'RNCc%u'),5)))
         set(lfm_fy_edit,'String',num2str(LNC(sscanf(get(gco,'Tag'),'RNCc%u'),6)))
         set(lfm_fz_edit,'String',num2str(LNC(sscanf(get(gco,'Tag'),'RNCc%u'),7)))
         set(lfm_mx_edit,'String',num2str(LNC(sscanf(get(gco,'Tag'),'RNCc%u'),8)))
         set(lfm_my_edit,'String',num2str(LNC(sscanf(get(gco,'Tag'),'RNCc%u'),9)))
         set(lfm_mz_edit,'String',num2str(LNC(sscanf(get(gco,'Tag'),'RNCc%u'),10)))
         set(lfm_alpha_edit,'String',num2str(LNC(sscanf(get(gco,'Tag'),'RNCc%u'),12)))
         if isequal(LNC(sscanf(get(gco,'Tag'),'RNCc%u'),13),2) ...
               || isequal(LNC(sscanf(get(gco,'Tag'),'RNCc%u'),13),3) ...
               || isequal(LNC(sscanf(get(gco,'Tag'),'RNCc%u'),13),4)
            if isequal(round(Thg(sscanf(get(gco,'Tag'),'RNCc%u'),1)*10^9)/10^9,0) && isequal(round(Thg(sscanf(get(gco,'Tag'),'RNCc%u'),2)*10^9)/10^9,0) ...
               ...
               set(lfm_height_edit,'Value',1) 
            else
               set(lfm_height_edit,'Value',LNC(sscanf(get(gco,'Tag'),'RNCc%u'),13))
            end
         else
            set(lfm_height_edit,'Value',1)
         end       
