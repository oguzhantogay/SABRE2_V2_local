function [slender,Mz_temp,yc_temp,Ixx,rt_temp,Shear_center_temp]=StiffnessELEMENT3DESL_slender(bfb,tfb,bft,tft,Dg,yc,ht,hb,Dt,Db,hst,...
    hsb,ys,tw,Afillet,L,E,G,fillet,hstp,hsbp,ysp,bftp,bfbp,...
    theta1x,theta1z,theta1y,theta2x,theta2z,theta2y,theta1xp,theta2xp,qno,Fy,Fyfi,Fyw,Fyfo,ytbar,Pbar,FEL,LIAType,HomoType,angletap)
% Developed by Woo Yong Jeong.
% Date : 12/01/2012.
% ************************************************************************
% ********       Slender Only for Elastic Analysis           *************
% ************************************************************************
...
De=Dg(1)*(1-xe/L)+Dg(2)*xe/L;  %web height
...
% Calculate Ixx
Ixx = bfte.*(tfte.^3)/12 + twe.*(De.^3)/12 + bfbe.*(tfbe.^3)/12+...
    Aft.*(hcft).^2+Afb.*(hcfb).^2+Aw.*(De/2-hcfb+tfbe/2).^2;
...
A_fc=bfcom.*tfcom;
D_temp=De;
h_temp=De+tfcom./2+tften./2;
d_temp=De+tfcom+tften;
rt_temp=bfcom./sqrt(12.*((h_temp./d_temp)+1/3.*(A_wc./A_fc).*(D_temp.^2)./(h_temp.*d_temp)));
Shear_center_temp=huft;
...
for i=1:nip
    ...
    elseif isequal(LIAType,1)
        lam_rw(1,i) = 3.1+2.5*bfcom(1,i)*tfcom(1,i)/( Dc(1,i)*twe(1,i));
        lam_rw(1,i) = max(4.6, min(5.7,lam_rw(1,i)))*sqrt(E/Fycom);
    end
end
