function [AI,gammma,FunewR,EGunew,Qintf,QintfE,Qint1,Qint2,MI,tau,Rpg,Rpc,Rpt,Rh,Myc,Myt,My,Jval,Phi_Mmax,Phi_Py,UC,crLTB,LGv] = AnalysisIEBuckling_ryan(Massemble,...
...
%       Rsd(3,4)=rd1(n);  Rsd(10,11)=rd2(n); 
   ...
   for n = 1:xn
      ...
      [tau(:,n),tau_a(:,n),Rpg(:,n),Rpc(:,n),Rpt(:,n),Rh(:,n),Myc(:,n),My(:,n),Myt(:,n),Phi_Mmax(:,n),...
          UC(:,n),Mmax(:,n),Py(:,n),psy(:,n),MmaxL(:,n),Mn_FLB(:,n),Mn_TFY(:,n)]=StiffnessELEMENT3DtauUC([bfb1(n);bfb2(n)],...
         [tfb1(n);tfb2(n)],[bft1(n);bft2(n)],[tft1(n);tft2(n)],...
         [Dg1(n);Dg2(n)],[ht1(n);ht2(n)],[hb1(n);hb2(n)], ...
         [ys1(n);ys2(n)],[tw1(n);tw2(n)],[Afillet1(n),Afillet2(n)],L0(n,1),E(n,1),fillet(n,:),...
   ...
   if isequal(flag,2) || isequal(FLflag,1) % Flexural Yielding or FLB or TFY control
      ...
      if max_UC >= 1
         ...
         if isequal(crLTB,1)
            ...
            LGv = gammmaIn*gammma;
%             set(pt_title_name,'String',['Inelastic Linear Buckling Analysis is completed. Applied Load Ratio = ',num2str(gammma(1,1)),' (FLB)',...
%                '.  Applied Load Ratio =',num2str(LGv),' (Buckling)'])
%             set(pt_title_name,'Visible','on')  
%             set(ap_cv_text,'String','FLB')  
         elseif isequal(crLTB,2)
            gammma=max(Fn3./Fn0)/max_UC;
            LGv = gammmaIn*gammma;
%             set(pt_title_name,'String',['Inelastic Linear Buckling Analysis is completed. Applied Load Ratio = ',num2str(gammma(1,1)),' (TFY)',...
%                '.  Applied Load Ratio =',num2str(LGv),' (Buckling)'])
%             set(pt_title_name,'Visible','on')  
%             set(ap_cv_text,'String','TFY')
         elseif isequal(crLTB,3)
            gammma=max(Fn3./Fn0)/max_UC;
            LGv = gammmaIn*gammma;
%             set(pt_title_name,'String',['Inelastic Linear Buckling Analysis is completed. Applied Load Ratio = ',num2str(gammma(1,1)),' (CFY)',...
%                '.  Applied Load Ratio =',num2str(LGv),' (Buckling)'])
%             set(pt_title_name,'Visible','on')   
%             set(ap_cv_text,'String','Yielding')
         end
      else
         Qintf=Qintf(:,:,end)*max_UC;
         QintfE=QintfE(:,:,end)*max_UC;
...
%             set(ap_cv_text,'String','Yielding')
      ...
      if gammmaIn >= 0.999 && gammmaIn <1.001  
         ...
         if isequal(mRatio_SRF,1) % Elastic buckling control under Mmax
            AI=2; % Elastic control
            gammma=max(Fn2./Fn0);
            Qintf=Qintf(:,:,end);
            QintfE=QintfE(:,:,end);
            Qint1=Qint1(:,:,end);
            Qint2=Qint2(:,:,end);
%             set(pt_title_name,'String',['Inelastic Linear Buckling Analysis is completed. Applied Load Ratio = ',num2str(gammma(1,1))])
%             set(pt_title_name,'Visible','on')  
%             set(ap_cv_text,'String','Elastic')               
             ...
             Qint1=Qint1(:,:,end);
             Qint2=Qint2(:,:,end);
%              set(ap_cv_text,'String','Converged')
%              set(pt_title_name,'String',['Inelastic Linear Buckling Analysis is completed. Applied Load Ratio = ',num2str(gammma(1,1))])   
%              set(pt_title_name,'Visible','on')  
         end
         ...
         AI =1;
%          set(ap_cv_text,'String','Non-converged')
%          set(pt_title_name,'String','Inelastic Linear Buckling Analysis did not finish. Please try to use the other Algorithm.')
%          set(pt_title_name,'Visible','on')
         gammma=0;   
         Qintf=Qintf(:,:,end)*0;
         QintfE=QintfE(:,:,end)*0;
