function [ANE,gammma,FunewR,EGunew,Qintf,QintfE,Qint1,Qint2,MI,PrPeLNB] = AnalysisNonBucklingESL(Massemble,...
...
% Date : 2/01/2015.
% ************************************************************************
% *******       Nonlinear Elastic Eigenvalue Buckling Load         *******
% ************************************************************************
% ************************** Wait1
wh = waitbar(0,'Initializing waitbar...');

set(get(ap_da_buttongroup,'SelectedObject'),'Tag','da_off');
...
PrPeLNB=zeros(5,xn,(round(ninc/lambda))); % Preallocationg
...
Lrd1=zeros(xn,1); 
for i=1:xn
   if isequal(LNC1(i,5),0) && isequal(LNC1(i,6),0)
      Lrd1(i,1)=0;
   else     
...
Lrd2=zeros(xn,1);
for i=1:xn
   if isequal(LNC2(i,5),0) && isequal(LNC2(i,6),0)
      Lrd2(i,1)=0;
   else    
...
if ~isempty(LUEC)
   for i=1:xn
      if isequal(LUEC(i,5),0) && isequal(LUEC(i,6),0)
         Lud1(i,1)=0;
      else       
...
% ------------------------------------------------------------------------
% ---------       Nonlinear Elastic Bucklling Analysis       -------------
% ------------------------------------------------------------------------
...
pn=0; nn=0; Sflag=0; gammap=[]; gamman=[];gamp=[]; gamn=[];
PrPeL=zeros(5,xn);
...
%    if isequal(increment,1)  
   ...
   %       Rsd(3,4)=rd1(n);  Rsd(10,11)=rd2(n); 
      ...
      tdus = KtBB\(Fn);     % 1st Elastic displacement

      % Restoration DOF of the nodal displacements considering BC
      [tdu,RTsuppt]=RTDisplacement(tdus,nDOF,BC);
      
      [Tunew]=CurrentDisplacement(tdu,N,nodedof); % including supports nodes;      
      Tunew1 = zeros(xn,7); Tunew2 = zeros(xn,7);
      for i=1:xn
         for j=1:7
            Tunew1(i,j)=Tunew(MI(i,2),j); 
            Tunew2(i,j)=Tunew(MI(i,3),j);
         end
      end     
   ...
   % -------------------------------------------------------- Slender S 
      ...
      for n = 1:xn
         ...
         [e1,e2,e3] = GlobalTransform(e1,e2,e3,alphatap(n,2));
         [Ge]=CoordTransform(e1,e2,e3); 
         qno=Pbar*Rd*Ge'*[Tunew1(n,:)';Tunew2(n,:)'];
   ...
   for n = 1:xn
      ...
      DE=zeros(14,1); SE=zeros(14,1);
      if ~isempty(LUEC)
         [Kt,Kgg,Qino,Kh2,Gn]=StiffnessELEMENT3DSlenderDM([bfb1(n);bfb2(n)],...
            [tfb1(n);tfb2(n)],[bft1(n);bft2(n)],[tft1(n);tft2(n)],...
            [Dg1(n);Dg2(n)],[yc1(n);yc2(n)],[ht1(n);ht2(n)],[hb1(n);hb2(n)], ...
            [Dt1(n);Dt2(n)],[Db1(n);Db2(n)],[hst1(n);hst2(n)],[hsb1(n);hsb2(n)], ...
            [ys1(n);ys2(n)],[tw1(n);tw2(n)],[Afillet1(n),Afillet2(n)],L0(n,1),E(n,1),G(n,1),fillet(n,:),...
            hstp(n),hsbp(n),ysp(n),bftp(n),bfbp(n), ...
            theta1xn,theta1zn,theta1yn,theta2xn,theta2zn,theta2yn,...
            theta1xpn,theta2xpn,qno,LUNCe(n,2),LUEC(n,17),FEL(n,:),slender(:,n),ap_da_buttongroup,taub(:,n));
         ...
         [Kt,Kgg,Qino,Kh2,Gn]=StiffnessELEMENT3DNSlenderDM([bfb1(n);bfb2(n)],...
            [tfb1(n);tfb2(n)],[bft1(n);bft2(n)],[tft1(n);tft2(n)],...
            [Dg1(n);Dg2(n)],[yc1(n);yc2(n)],[ht1(n);ht2(n)],[hb1(n);hb2(n)], ...
            [Dt1(n);Dt2(n)],[Db1(n);Db2(n)],[hst1(n);hst2(n)],[hsb1(n);hsb2(n)], ...
            [ys1(n);ys2(n)],[tw1(n);tw2(n)],[Afillet1(n),Afillet2(n)],L0(n,1),E(n,1),G(n,1),fillet(n,:),...
            hstp(n),hsbp(n),ysp(n),bftp(n),bfbp(n), ...
            theta1xn,theta1zn,theta1yn,theta2xn,theta2zn,theta2yn,...
            theta1xpn,theta2xpn,qno,FEL(n,:),slender(:,n),ap_da_buttongroup,taub(:,n));   
      end
...
%          Rsd(3,4)=rd1(n);  Rsd(10,11)=rd2(n); 
         ...
         Qin(n,:) = QeiD(:);   
         [PrPeL(:,n)]=StiffnessELEMENT3DPePel([bfb1(n);bfb2(n)],...
           [tfb1(n);tfb2(n)],[bft1(n);bft2(n)],[tft1(n);tft2(n)],...
           [Dg1(n);Dg2(n)],[yc1(n);yc2(n)],[ht1(n);ht2(n)],[hb1(n);hb2(n)], ...
           [Dt1(n);Dt2(n)],[Db1(n);Db2(n)],[hst1(n);hst2(n)],[hsb1(n);hsb2(n)], ...
           [ys1(n);ys2(n)],[tw1(n);tw2(n)],[Afillet1(n),Afillet2(n)],L0(n,1),E(n,1),fillet(n,:),...
           [ytbar1(n);ytbar2(n)],Qin(n,:));  
...
%       norm = sqrt(R'*R);
   ...
   for i = 1:xn
      ...
      for i = 1:xn
          for j = 1:5
                  PrPeLNB(j,i,increment) = PrPeL(j,i);
          end
      end 
   % ------------------------------------------------------------- Nominal Stiffness S
...
%       [qno] = ElDisplacement(theta1xn,theta1zn,theta1yn,theta2xn,theta2zn,theta2yn,theta1xpn,theta2xpn,ul(n,1));
      ...
      if ~isempty(LUEC)
         ...
         [Kt,Kgg,Qino,Kh2,Gn]=StiffnessELEMENT3DNSlender([bfb1(n);bfb2(n)],...
            [tfb1(n);tfb2(n)],[bft1(n);bft2(n)],[tft1(n);tft2(n)],...
            [Dg1(n);Dg2(n)],[yc1(n);yc2(n)],[ht1(n);ht2(n)],[hb1(n);hb2(n)], ...
            [Dt1(n);Dt2(n)],[Db1(n);Db2(n)],[hst1(n);hst2(n)],[hsb1(n);hsb2(n)], ...
            [ys1(n);ys2(n)],[tw1(n);tw2(n)],[Afillet1(n),Afillet2(n)],L0(n,1),E(n,1),G(n,1),fillet(n,:),...
            hstp(n),hsbp(n),ysp(n),bftp(n),bfbp(n), ...
            theta1xn,theta1zn,theta1yn,theta2xn,theta2zn,theta2yn,...
            theta1xpn,theta2xpn,qno,FEL(n,:),slender(:,n));  
      end
...
%    set(am_cv_text,'String','Unstable')
   set(pt_title_name,'String','Elastic Nonlinear Buckling Analysis did not finish. Please try to check loads and boundry conditions.')
   ...
   Qint1=Qint1(:,:,end)*0;
   Qint2=Qint2(:,:,end)*0;
   PrPeLNB = PrPeLNB(:,:,end)*0;
elseif isequal(Bflag,2) 
   delete(wh)
%    set(am_cv_text,'String','Non-converged')
   set(pt_title_name,'String','Elastic Nonlinear Buckling Analysis did not finish. Please try to use the other Algorithm or Sacaled Parameter,')  
   set(pt_title_name,'Visible','on')
   gammma=0;   
   Qintf=Qintf(:,:,end)*0;
   QintfE=QintfE(:,:,end)*0;
   ...
   if gammmaIn >= 0.999 && gammmaIn <1.001
      if norm > 0.1
         ...
         Qint2=Qint2(:,:,end)*0;   
         PrPeLNB = PrPeLNB(:,:,end)*0;
%          set(am_cv_text,'String','Diverged')
         set(pt_title_name,'String','Elastic Nonlinear Buckling Analysis did not complete.')           
      else
         gammma=max(Fn2./Fn0);   
         Qintf=Qintf(:,:,end);
         QintfE=QintfE(:,:,end);
...
%          set(am_cv_text,'String','Converged')
         set(pt_title_name,'String',['Elastic Nonlinear Buckling Analysis is completed. Applied Load Ratio = ',num2str(gammma(1,1))])
      end            
   else 
%       set(am_cv_text,'String','Non-converged')
      set(pt_title_name,'String','Elastic Nonlinear Buckling Analysis did not finish. Please try to use the other Algorithm')
      gammma=0;   
      Qintf=Qintf(:,:,end)*0;
      QintfE=QintfE(:,:,end)*0;
      Qint1=Qint1(:,:,end)*0;
...
ANE=1; % Set nonlinear elastic analysis