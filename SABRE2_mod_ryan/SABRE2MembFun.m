function SABRE2MembFun(pn_panel,pt_title_name,fim_infor_name,pd_type_subpanel,pdn_type_text,...
...
% *** sub panel1
set([pde_type_text,pde_type_name],'Visible','on')
% *** sub panel2
set(pde_buttongroup,'Visible','on');
set([pde_member_text,pde_jointi_radiobutton,pde_jointj_radiobutton],'Visible','on');
set([pde_jointi_edit,pde_jointj_edit],'Visible','on'); 
set([pde_reference_text,pde_reference_edit],'Visible','on');
% *** sub panel3
set(pde_subpanel,'Visible','on')
set([pde_AISC_text,pde_wsection_text],'Visible','on');
set([pde_wsection_edit,pde_set,pde_wsname_edit],'Visible','on');
% *** sub panel4
set(pdm_subpanel,'Visible','on');
set(pde_section_text,'Visible','on');
set([pde_ji_text,pde_jj_text],'Visible','on');
set([pde_bfb_text,pde_tfb_text,pde_bft_text],'Visible','on');
set([pde_tft_text,pde_dw_text,pde_tw_text],'Visible','on');
set([pde_bfbi_edit,pde_tfbi_edit,pde_bfti_edit,pde_tfti_edit,pde_dwi_edit,pde_twi_edit],'Visible','on');
set([pde_bfbj_edit,pde_tfbj_edit,pde_bftj_edit,pde_tftj_edit,pde_dwj_edit,pde_twj_edit],'Visible','on'); 
set([pde_fil_text,pde_fili_edit,pde_filj_edit],'Visible','on');
% *** sub panel5
set([pde_apply,pde_cancel],'Visible','on'); 
...
% -----------------------        EDIT INUPT       ------------------------
% ------------------------------------------------------------------------
set(findobj('Color','r'),'HitTest','on');
set(findobj('Tag','M'),'HitTest','on');  

% Select two Joint nodes for Each member  
switch get(get(pde_buttongroup,'SelectedObject'),'Tag')
   case 'joint_i'
      if strcmp(get(gco,'type'),'line')
         if strcmp(sscanf(get(gco,'Tag'),'%[jn]'),'jn')                        
            set(findobj('Color','g'),'Color','r')
            ...
            set(gco,'Color','g')            
            set(pde_jointi_edit,'String',num2str(sscanf(get(gco,'Tag'),'jn%u')));
            set(pde_jointi_radiobutton,'value',0)
            set(pde_jointj_radiobutton,'value',1)              
         end   
      end  
   ...
   case 'joint_j'
      if strcmp(get(gco,'type'),'line')
         if strcmp(sscanf(get(gco,'Tag'),'%[jn]'),'jn')
            ...
            set(gco,'Color','g')           
            set(pde_jointj_edit,'String',num2str(sscanf(get(gco,'Tag'),'jn%u')));
...
if strcmp(get(gco,'type'),'text')
   if strcmp(sscanf(get(gco,'String'),'%[M]'),'M')
      ...
      set(pde_type_name,'String',num2str(mnum));
      set(pde_jointi_edit,'String',num2str(Massemble(mnum,2)));
      set(pde_jointj_edit,'String',num2str(Massemble(mnum,3))); 
      set(pde_bfbi_edit,'String',num2str(JNodevalue_i(mnum,6)));
      set(pde_tfbi_edit,'String',num2str(JNodevalue_i(mnum,7))); 
      set(pde_bfti_edit,'String',num2str(JNodevalue_i(mnum,8)));
      set(pde_tfti_edit,'String',num2str(JNodevalue_i(mnum,9))); 
      set(pde_dwi_edit,'String',num2str(JNodevalue_i(mnum,10)));
      set(pde_twi_edit,'String',num2str(JNodevalue_i(mnum,11)));
      set(pde_fili_edit,'String',num2str(JNodevalue_i(mnum,14)));
      set(pde_bfbj_edit,'String',num2str(JNodevalue_j(mnum,6)));
      set(pde_tfbj_edit,'String',num2str(JNodevalue_j(mnum,7))); 
      set(pde_bftj_edit,'String',num2str(JNodevalue_j(mnum,8)));
      set(pde_tftj_edit,'String',num2str(JNodevalue_j(mnum,9))); 
      set(pde_dwj_edit,'String',num2str(JNodevalue_j(mnum,10)));
      set(pde_twj_edit,'String',num2str(JNodevalue_j(mnum,11)));
      set(pde_filj_edit,'String',num2str(JNodevalue_j(mnum,14)));

      if ~isempty(Rval)
         set(pde_reference_edit,'Value',Rval(mnum,2));
      end
...
set(pde_wsection_edit,'String',num2str(TYPE))

if isempty(get(pde_bfbi_edit,'String')) ...
...
set(pde_wsname_edit,'String','');    
   ...
   if isequal(dunit,2)
      ...
      for i=1:(length(SECTION(:,1)))
         if isequal(SECTION(i,1)*2.54,str2double(get(pde_bfbi_edit,'String'))) && isequal(SECTION(i,2)*2.54,str2double(get(pde_tfbi_edit,'String'))) ... 
               && isequal(SECTION(i,3)*2.54,str2double(get(pde_bfti_edit,'String'))) && isequal(SECTION(i,4)*2.54,str2double(get(pde_tfti_edit,'String'))) ...
               && isequal(round((SECTION(i,5)*2.54)*10^5)/10^5,round( ( str2double(get(pde_dwi_edit,'String'))  + str2double(get(pde_tfbi_edit,'String')) + str2double(get(pde_tfti_edit,'String'))  )*10^5)/10^5) ...
               && isequal(SECTION(i,6)*2.54,str2double(get(pde_twi_edit,'String'))) ...
               && isequal(SECTION(i,1)*2.54,str2double(get(pde_bfbj_edit,'String'))) && isequal(SECTION(i,2)*2.54,str2double(get(pde_tfbj_edit,'String'))) ...
               && isequal(SECTION(i,3)*2.54,str2double(get(pde_bftj_edit,'String'))) && isequal(SECTION(i,4)*2.54,str2double(get(pde_tftj_edit,'String'))) ...
               && isequal(round((SECTION(i,5)*2.54)*10^5)/10^5,round( ( str2double(get(pde_dwj_edit,'String'))  + str2double(get(pde_tfbj_edit,'String')) + str2double(get(pde_tftj_edit,'String'))  )*10^5)/10^5) ...
               && isequal(SECTION(i,6)*2.54,str2double(get(pde_twj_edit,'String'))) ... 
               && isequal(round((SECTION(i,7)*2.54*2.54)*10^5)/10^5,round( ( str2double(get(pde_bfbi_edit,'String'))...
               *str2double(get(pde_tfbi_edit,'String')) +str2double(get(pde_bfti_edit,'String'))*str2double(get(pde_tfti_edit,'String'))...
               ...
               && isequal(round((SECTION(i,7)*2.54*2.54)*10^5)/10^5,round( ( str2double(get(pde_bfbj_edit,'String'))...
            ...
            set(pde_wsname_edit,'String',num2str(TYPE((i+1),:)));
               break;
